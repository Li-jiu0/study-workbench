...
var AI_CONFIG = {
  ...
  providers: {
    ...
    gemini: {
      ...
      // R11g（2026-09-21 故障修复）：此处曾有 apiKey 字段，R131 去明文 key 时其余 6 家
      // 都把字段【整行删除】，唯独 gemini 只把值换成了作废占位串。而 ai-service.js 的
      // xtResolveChannel() 判「provider.apiKey 非空 → relay:false 走前端直连」，
      // 于是一个**只作废值、字段还在**的残留会让 gemini 绕过服务端中转，
      // 拿作废串直连 Google → 401/403 → 连通性检测误报「Key 无效或已过期」。
      // 故此处必须保持「字段不存在」，与其余 6 家一致；用户自备 Key 走
      // localStorage 的 ai_user_key_gemini（BYOK 回退分支），绝不写回本文件。
      apiFormat: "gemini",
      keyInQuery: true,
      needVPN: true,
      needProxy: true
  ...
  builtinModels: [
    ...
    },
    {
      ...
      provider: "ark",
      model: "doubao-smart-router-250928",
      types: ["general"],
      tag: "免费",
      rate: "1x",
      temperature: 0.7,
    ...
    /* R11d（2026-09-21 用户要求）：视频能力重新接入 —— seedance 1.0 pro / pro-fast。
       已用 server/.env 的 ARK_API_KEY 核对 GET /v3/models：两者均「无 status = 在服可调用」。
       audio:true 是设置页「有声」小开关的数据源，不可省略。 */
    {
    ...
    // R11c（2026-09-21 用户要求）：接入 Doubao-Seedream-5.0。
    // 已用 server/.env 的 ARK_API_KEY 实测 doubao-seedream-5-0-260128 → images/generations
    // HTTP 200 正常出图；请求体形状（model / prompt / image_size / n / response_format）与
    // 4.x 完全一致，链路无需任何改动，也不会触发 4.5 的「尺寸下限」问题。
    {
      id: "ark-seedream-5-0",
      name: "Seedream-5.0",
      provider: "arkimage",
  ...
  modelDetails: {
    ...
    "ark-seedream-5-0": { platform: "火山方舟·图片生成", params: "", type: "图片生成（最新）", stars: 5, speed: "快", advantage: "Seedream 5.0 文生图，画质与指令跟随较 4.x 全面提升；走 images/generations 接口，实测 1024x1024 直连出图正常", applicable: "文生图（首选）" },
    "ark-seedream-5-0-pro": { platform: "火山方舟·图片生成", params: "", type: "图片生成（Pro）", stars: 5, speed: "中", advantage: "Seedream 5.0 Pro 文生图，复杂提示词与画质更强；走 images/generations 接口，实测直连 HTTP 200 出图", applicable: "文生图（质量优先）" },
    ...
    /* R11d（2026-09-21 用户要求）：视频 / 新模型明细重新接入。
       seedance 1.0 pro / pro-fast 明细随模型一并回填；seedream-5-0-pro / hyper3d-gen2 /
       seed-2.1-pro-260628 / seed-2.0-pro / smart-router 明细同步补齐。 */
    "ark-seedance-1-0-pro": { platform: "火山方舟", params: "", type: "视频生成", stars: 5, speed: "慢", advantage: "文生视频 / 图生视频；输入文字提示词（图生视频再传一张首帧图），输出 5～10 秒 MP4；分钟级异步返回；单次消耗约 10 万 tokens 量级", applicable: "文生视频、图生视频" },
    "ark-seedance-1-0-pro-fast": { platform: "火山方舟", params: "", type: "视频生成（快速版）", stars: 4, speed: "慢", advantage: "文生视频 / 图生视频快速版，输入与输出同标准版，出片更快；分钟级异步返回", applicable: "文生视频、图生视频（速度优先）" },
    ...
    "ark-seed-2-1-pro-260628": { platform: "火山方舟", params: "", type: "通用对话（新版）", stars: 5, speed: "中", advantage: "Doubao-Seed-2.1-pro 的 260628 版本；额度每天 200 万 token", applicable: "复杂问答、长链思考" },
    "ark-seed-2-0-pro": { platform: "火山方舟", params: "", type: "通用对话", stars: 4, speed: "中", advantage: "Doubao-Seed-2.0-pro；额度每天 200 万 token", applicable: "日常问答、文案创作" },
    "ark-smart-router": { platform: "火山方舟", params: "", type: "自动路由", stars: 4, speed: "快", advantage: "方舟智能路由模型，自动选择合适模型作答", applicable: "不想挑模型时的日常问答" },
  ...
  FUNC_TYPES: {
    ...
    imagegen: {
      ...
      // R11c（2026-09-21）：接入 Seedream-5.0 后升为 primary（实测 200 出图），
      // 4.0-Fast / 4.0 降为 fallback 兜底。
      primary: "ark-seedream-5-0",
      fallback: ["ark-seedream-5-0-pro", "ark-seedream-4-0828", "ark-seedream-4-0415"],
      temperature: 0.8,
      maxTokens: 1000
    },
    audio: {
    ...
    video: {
      desc: "视频生成",
      /* R11d（2026-09-21 用户要求）：视频能力重新接入 —— seedance 1.0 pro / pro-fast。
         R11b 的占位（primary:null / fallback:[]）已回填为下列链。 */
    ...
    three_d: {
      ...
      fallback: ["ark-hyper3d-gen2", "ark-hitem3d-2-0"],
      temperature: 0.7,
      maxTokens: 1000
    }
  }

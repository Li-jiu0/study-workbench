(function () {
  ...
  function boot() {
    ...
    window.addEventListener('pagehide', function () {
      try {
        if (VR.stream) { VR.stream.getTracks().forEach(function (t) { try { t.stop(); } catch (e1) { } }); }
        VR.stream = null;
      } catch (e2) { }
      try { if (VR.rec && VR.rec.state === 'recording') { VR.rec.stop(); } } catch (e3) { }
      try { if (window.XTAppBridge && typeof window.XTAppBridge.stopVoiceRecord === 'function') window.XTAppBridge.stopVoiceRecord(); } catch (e4) { }
    });

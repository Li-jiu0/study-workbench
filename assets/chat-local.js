(function () {
  ...
  /* R3b-C U9：待发送引用草稿（选中「引用」后挂起，发送完成 / 切会话 / 点 × 均清空）。
     结构：{ toId, senderName, kind, text, name }（name 为展示用被引用者昵称，可能含本端备注）。 */
  S.replyDraft = null;
  ...
  window.imRecallPressStart = function (id, e, el) {
    /* R3b-C U9（2026-09-20）：图片气泡纳入长按菜单 —— 修复 R73 需求3「图片气泡不参与长按菜单」。
       原顾虑是「慢点一下被吞成菜单而非打开图片预览」。现解法：长按判定仍是 500ms 且容许 10px 抖动，
       轻点（未达 500ms）在 touchend 即取消计时、图片的 onclick 预览照常触发；只有真正按住 500ms
       才唤起菜单（引用 / 撤回 / 删除本端）。故图片既能点开预览、也能长按引用，不再互相吞没。 */
    imClearPress();
  ...
  /* ==================== R3b-C U9（2026-09-20）：长按引用 / 回复 ====================
     微信式引用：长按（或右键）任意消息（本人 / 对方、文本 / 图片 / 语音 / 位置 / 文件），
     菜单选「引用」→ 输入框上方出现引用条 → 发送的新消息带引用块 → 对方可见、可点引用块跳回原消息。
     设计要点：
       - 引用快照随消息走（后端 reply_to_id / reply_sender_name / reply_kind / reply_text 四列）；
       - 本端乐观气泡也带同一份快照（引用块立即可见）；
       - 被引用消息被「删除本端 / 撤回」或拉取不到 → 引用块显示「原消息已撤回 / 已删除」，不崩不留白。 */

  /* 被引用消息的内容摘要（≤100 字），口径与后端 _reply_preview 对齐：
     图片「[图片]」/ 语音「[语音]」/ 位置「[位置] 地名」/ 文件「[文件] 名」/ 文本正文前 100 字。 */
  function imReplySummary(m) {
    if (!m) return '[消息]';
    var k = m.kind || 'text';
    var c = (m.content == null ? '' : String(m.content)).replace(/^\s+|\s+$/g, '');
    if (k === 'image') return '[图片]';
    if (k === 'voice') return '[语音]';
    if (k === 'location_live') return '[实时位置]';
    if (k === 'location') return c ? ('[位置] ' + c).slice(0, 100) : '[位置]';
    if (k === 'file') { var fn = m.name || c; return fn ? ('[文件] ' + fn).slice(0, 100) : '[文件]'; }
    return c ? c.slice(0, 100) : '[消息]';
  }

  /* 被引用者的展示名：自己 → 「我」；他人 → 群名片 / 备注名 / 昵称（复用既有命名口径）。
     私聊对方取 S.peer 的备注名；群聊取消息自带 senderNickname + 本端群备注。 */
  function imReplySenderName(m) {
    if (!m) return '';
    if (m.senderId === S.myId) return '我';
    if (m.senderNickname) {
      return imFriendName({ peerRemark: imRemarkOf(m.senderId), nickname: m.senderNickname }, '好友');
    }
    if (S.peer && m.senderId === S.peer.serverId) {
      return imFriendNameOf(S.peer, '好友');
    }
    return m.senderNickname || '好友';
  }

  /* 由一条消息构造待发引用草稿（供菜单「引用」项调用）。 */
  function imBuildReplyRef(m) {
    if (!m || m.id == null) return null;
    return {
      toId: m.id,
      senderName: imReplySenderName(m),
      kind: m.kind || 'text',
      text: imReplySummary(m),
    };
  }

  /* 选中「引用」：挂起草稿 + 显示输入框上方引用条 + 输入框获焦。 */
  window.imStartReply = function (id) {
    var m = imFindMsg(id);
    if (id != null && !m) {
      // 兜底：传 id 但本地找不到（如已删除本端）→ 允许用 id 占位引用（快照降级）
      m = { id: id, kind: 'text', content: '', senderId: 0 };
    }
    var ref = imBuildReplyRef(m);
    if (!ref) return;
    S.replyDraft = ref;
    imRenderQuoteBar();
    var inp = $id('imInput');
    if (inp) { try { inp.focus(); } catch (e) { /* 老 WebView 忽略 */ } }
  };

  /* 渲染 / 隐藏输入框上方引用条（依据 S.replyDraft）。 */
  function imRenderQuoteBar() {
    var bar = $id('imQuoteBar');
    if (!bar) return;
    var d = S.replyDraft;
    if (!d) { bar.classList.remove('on'); return; }
    var nm = $id('imQuoteName'), tx = $id('imQuoteText');
    if (nm) nm.textContent = d.senderName || '';
    if (tx) tx.textContent = d.text || '';
    bar.classList.add('on');
  }

  /* 清空待发引用（点 × / 发送完成 / 切换会话均调用）。 */
  window.imClearReplyDraft = function () {
    S.replyDraft = null;
    imRenderQuoteBar();
  };

  /* 取出本次发送要携带的 reply 对象（无草稿 → null）。发送前快照，避免发送过程中被清空。 */
  function imReplyPayload() {
    var d = S.replyDraft;
    if (!d) return null;
    return { toId: d.toId, senderName: d.senderName || '', kind: d.kind || 'text', text: d.text || '' };
  }

  /* 消息气泡顶部引用块 HTML（点击 → 平滑滚动并高亮原消息）。
     被引用消息当前不可见（本端已删 / 已撤回 / 不在列表）→ 显示「原消息已撤回 / 已删除」占位。 */
  function imReplyBlockHtml(reply) {
    if (!reply) return '';
    var toId = reply.toId;
    var key = imThreadKey();
    var name = esc(reply.senderName || '');
    var gone = '';
    if (imIsDeleted(key, toId)) gone = '原消息已删除';
    else if (imIsRecalled(key, toId)) gone = '原消息已撤回';
    else if (String(reply.text || '') === '') gone = '原消息已删除';
    var tx = gone ? gone : esc(reply.text || '');
    var cls = 'im-reply' + (gone ? ' im-reply-gone' : '');
    return '<div class="' + cls + '" onclick="imJumpToMsg(event,\'' + esc(toId) + '\')">' +
      '<div class="im-reply-nm">' + (name || '消息') + '</div>' +
      '<div class="im-reply-tx">' + tx + '</div>' +
      '</div>';
  }

  /* 点击引用块：平滑滚动定位到原消息并高亮几秒。
     原消息不在当前 DOM（已删 / 已撤回 / 更早未加载）→ toast 提示，不抛异常。 */
  window.imJumpToMsg = function (ev, id) {
    if (ev && typeof ev.stopPropagation === 'function') ev.stopPropagation();
    var box = $id('imMsgs');
    if (!box) return;
    var key = imThreadKey();
    if (imIsDeleted(key, id)) { toast('原消息已删除'); return; }
    if (imIsRecalled(key, id)) { toast('原消息已撤回'); return; }
    var el = box.querySelector('[data-msg-id="' + String(id).replace(/"/g, '\\"') + '"]');
    if (!el) { toast('原消息已不在当前会话'); return; }
    try {
      el.scrollIntoView({ behavior: 'smooth', block: 'center' });
    } catch (e) {
      // 老 WebView 不支持 options → 回退瞬时滚动
      try { el.scrollIntoView(); } catch (e2) { /* 忽略 */ }
    }
    el.classList.add('im-reply-hl');
    setTimeout(function () { el.classList.remove('im-reply-hl'); }, 2000);
  };

  /* ==================== R3b-C U3（2026-09-20）：发送失败 → 重发 ====================
     失败消息不丢：气泡置失败态（红色感叹号 + 「重发」），点重发按原内容重走发送链路。
     图片 / 语音复用已选文件 / 录音（原始 File/Blob 暂存在消息对象 _file/_blob 上，不进 localStorage）。
     发送中置灰防重复；成功后清除失败态。 */
  var _imFailed = {};      // id -> 重发闭包 { retry: fn, busy: bool }
  var _imRetryFiles = {};  // id -> { file: Blob } 内存暂存原始二进制（不落盘）

  /* 标记某条本端消息发送失败（气泡置失败态）。 */
  function imMarkMsgFailed(id, retryFn, blobHolder) {
    if (id == null) return;
    _imFailed[String(id)] = { retry: retryFn, busy: false };
    if (blobHolder) _imRetryFiles[String(id)] = blobHolder;
    var list = S.msgs || [];
    for (var i = 0; i < list.length; i++) {
      if (String(list[i].id) === String(id)) { list[i].failed = true; break; }
    }
    if (typeof renderMsgs === 'function') renderMsgs();
  }

  /* 清除某条消息的失败态（重发成功后）。 */
  function imClearMsgFailed(id) {
    if (id == null) return;
    delete _imFailed[String(id)];
    delete _imRetryFiles[String(id)];
    var list = S.msgs || [];
    for (var i = 0; i < list.length; i++) {
      if (String(list[i].id) === String(id)) { list[i].failed = false; break; }
    }
  }

  /* 从 S.msgs 摘除某条失败消息（重发成功、服务端回包已顶替时）。 */
  function imDropMsg(id) {
    S.msgs = (S.msgs || []).filter(function (x) { return String(x.id) !== String(id); });
  }

  /* 点「重发」：置灰防重复 → 复用原内容重走发送（重发闭包在 imMarkMsgFailed 时已注册）。 */
  window.imRetryMsg = function (ev, id) {
    if (ev && typeof ev.stopPropagation === 'function') ev.stopPropagation();
    var rec = _imFailed[String(id)];
    if (!rec || rec.busy) return;   // 发送中置灰：忽略重复点击
    rec.busy = true;
    ...
    try {
      ...
      toast('重发失败：' + (e && e.message ? e.message : '未知错误'));
    }
  };
  ...
  window.imMenuQuote = function (id) {
    imCloseMsgMenu();
    window.imStartReply(id);
  };
  ...
  window.imOpenChat = function (friendId) {
    ...
    imClearReplyDraft(); // R3b-C U9：切换会话 → 清空待发引用草稿
    ...
    imKbOnEnter(); // R3-L1 问题三：进入会话 → 绑键盘避让监听
  ...
  /* R73 需求18③（2026-09-15）：普通用户可达的备注入口（聊天头部「✎」按钮）。
     ...
     R143-1（2026-09-21 用户要求）：「✎」改为**进入对方的个人资料页**（TA 视角），
     返回时经 ?from=私聊 深链回到当前会话（见 xt-profile.js otherReferrer 的白名单）。
     备注能力不丢：个人资料页 TA 视角自带「备注名」编辑入口（R74-B）。
     仅服务器好友有个人资料页；本地演示会话保留原「设置备注」弹窗（不删原功能）。 */
  function imEnsureRemarkBtn(show) {
    ...
    if (!btn) {
      ...
      btn.title = '查看资料';
      btn.textContent = '✎';
      btn.onclick = function () { imOpenPeerProfile(); };
  ...
  /* R143-1：头部「✎」新行为 —— 跳对方个人资料页（TA 视角），并携带回跳深链参数：
     user=对方uid（个人资料页用）+ from=私聊&uid&name（返回键回跳私聊并直接重开本会话）。 */
  function imPeerProfileUrl(uid, nm) {
    return '个人资料.html?user=' + encodeURIComponent(uid) +
      '&from=' + encodeURIComponent('私聊') +
      '&uid=' + encodeURIComponent(uid) +
      (nm ? '&name=' + encodeURIComponent(nm) : '');
  }
  function imOpenPeerProfile() {
    if (S.group) return;
    if (!S.peer || !S.peer.isServer || !(Number(S.peer.serverId) > 0)) {
      imOpenRemarkForCurrentChat();                        // 本地演示会话：回退原备注弹窗
      return;
    }
    location.href = imPeerProfileUrl(Number(S.peer.serverId), imFriendNameOf(S.peer, '') || '');
  }
  window.imPeerProfileUrl = imPeerProfileUrl;              // 测试钩子
  window.imOpenPeerProfile = imOpenPeerProfile;            // 测试钩子
  ...
  function renderChatHeader() {
    ...
    if (S.group) {
      ...
      if (nameEl) nameEl.innerHTML = esc(S.group.name) +
        '<span style="font-size:12px;color:#667eea;background:#EEF1FF;padding:1px 6px;border-radius:4px;margin-left:6px;font-weight:400">' + esc(String(S.group.memberCount || '')) + '人群</span>';
      // T02 增量：群会话显示「⋯」群设置入口
    ...
    if (nameEl) {
      ...
      var tag = aiCfg && aiCfg.apiKey
        ? '<span style="font-size:12px;color:#4caf50;background:#E8F5E9;padding:1px 6px;border-radius:4px;margin-left:6px;font-weight:400">AI在线</span>'
        : '';
  ...
  function backToList() {
    ...
    imKbOnLeave(); // R3-L1 问题三：退出会话 → 清零避让样式
    S.peer = null;
  ...
  function renderMsgs() {
    ...
    box.innerHTML = S.msgs.map(function (m) {
      ...
      /* R3b-C U9：气泡顶部引用块（本人 / 对方消息都渲染；无 reply 时为空串 → 现状完全一致）。 */
      var replyHtml = imReplyBlockHtml(m.reply);
      /* R3b-C U3：发送失败态 —— 红色感叹号 + 「重发」按钮（仅本端待发失败消息 m.failed 为真）。
         失败气泡内容保留不丢，点「重发」按原内容重走发送链路。 */
      var failHtml = '';
      if (m.failed && isMe) {
        failHtml = '<div class="im-fail"><span class="im-fail-ico">' +
          ((typeof window.lucideIcon === 'function') ? window.lucideIcon('alert-triangle', 16) : '!') +
          '</span><span>发送失败</span>' +
          '<span class="im-fail-re" onclick="imRetryMsg(event,\'' + esc(m.id) + '\')">重发</span></div>';
      }
      var bubbleCls = 'im-m ' + (isMe ? 'me' : 'ot') + (isMedia ? ' media' : '') + (m.failed && isMe ? ' im-msg-failed' : '');
      var body = '<div class="' + bubbleCls + '"' + imMsgAttrs(m, isMe) + ' data-mid="' + esc(m.id || '') + '">' + replyHtml + inner + failHtml + '</div>';
    ...
    fetch(apiBase() + '/api/uploads/image', {
      ...
      .then(function (res) {
        if (!res.ok || !res.d || !res.d.url) {
          if (cb) cb(null, (res.d && res.d.detail) || '图片上传失败');
          else toast((res.d && res.d.detail) || '图片上传失败');
          return;
        }
        if (cb) cb(res.d.url, null);
      })
      .catch(function (e) { if (cb) cb(null, '图片上传失败：' + (e.message || '网络错误')); else toast('图片上传失败：' + (e.message || '网络错误')); });
  }

  /* 上传成功后把 url 作为一条 kind='image' 消息发出去（群聊 / 私聊各一条分支）。
     发送成功后立刻重拉一次，服务端消息会覆盖掉先前的乐观气泡（与 postVoiceMsg 同节奏）。
     R3b-C U9/U3：携带引用快照；发送失败 → 乐观气泡转失败态 + 点「重发」复用 File 重走上传发送。 */
  function imPostImageMsg(url, replyRef, localMsgId, file) {
    var gid = S.group ? S.group.id : 0;
    var peerId = S.peer ? S.peer.serverId : 0;
    var doSendImg = function () {
      var body = { content: url, kind: 'image' };
      if (replyRef) body.reply = replyRef;
      var urlPath = S.group ? ('/api/groups/' + gid + '/messages') : ('/api/chat/' + peerId + '/messages');
      return fetch(apiBase() + urlPath, {
        ...
        .then(function (m) {
          imClearMsgFailed(localMsgId); imDropMsg(localMsgId);
          if (m && m.id) {
            ...
            if (m.reply) rec.reply = m.reply; else if (replyRef) rec.reply = replyRef;
            S.msgs.push(rec);
            renderMsgs();
          }
          if (S.group) fetchGroupMsgs(true); else fetchPeerMsgs(true);
        })
        .catch(function (e) {
          // 失败：置失败态，点「重发」复用原 File 重走「上传 → 发送」
          imMarkMsgFailed(localMsgId, function () {
            if (file) {
              imUploadImage(file, function (u, err) {
                if (!u) { toast(err || '图片上传失败'); return; }
                imPostImageMsg(u, replyRef, localMsgId, file);
              });
            } else {
              imPostImageMsg(url, replyRef, localMsgId, null);
            }
          }, file ? { file: file } : null);
          toast('图片发送失败：' + (e && e.message ? e.message : '网络错误') + '，点可重发');
        });
    };
    doSendImg();
  }
  ...
  function imSendImageFile(file) {
    ...
    /* R3b-C U9：图片也携带引用（发送前快照 + 清空草稿）。 */
    var replyRef = imReplyPayload();
    imClearReplyDraft();
    // 乐观渲染：先用本地 dataURL 顶上（上传有网络延迟），服务端消息回来后覆盖。
    // R3b-C U3：乐观气泡改用稳定 uid（供失败态 / 重发定位），不再是 tmp 时间戳。
    var uid = genMsgId();
    var rd = new FileReader();
    rd.onload = function (e) {
      var op = { id: uid, senderId: S.myId, content: e.target.result, kind: 'image', time: Date.now(), pendingSend: true };
      if (replyRef) op.reply = replyRef;
      S.msgs.push(op);
      renderMsgs();
    };
    rd.readAsDataURL(file);
    imUploadImage(file, function (url, err) {
      if (url) { imPostImageMsg(url, replyRef, uid, file); return; }
      // 上传即失败：乐观气泡转失败态，点「重发」复用原 File 重走上传 → 发送
      imMarkMsgFailed(uid, function () {
        imUploadImage(file, function (u2, e2) {
          if (!u2) { toast(e2 || '图片上传失败'); return; }
          imPostImageMsg(u2, replyRef, uid, file);
        });
      }, { file: file });
      toast(err || '图片上传失败，点消息可重发');
    });
  }
  ...
  /* R11 防漏释放：统一回收本页录音流的所有音轨（幂等，可重复调用）。
     凡是「拿到 stream 却没进入录制」的分支都必须走这里，否则麦克风会被本页自己焊死。 */
  function releaseRecStream() {
    try {
      if (VR.stream && VR.stream.getTracks) {
        VR.stream.getTracks().forEach(function (t) { try { t.stop(); } catch (e) { } });
      }
    } catch (e1) { }
    VR.stream = null;
  }
  ...
  /* ---------- R106b：起（L1/L2）—— 与点击切换共用 ----------
     ...
     isRetry=true 表示本次是「占用自愈」后的自动重试，再失败就不再重试（防死循环）。 */
  function recBegin(cb, isRetry) {
    if (voiceSupported()) {
      navigator.mediaDevices.getUserMedia({ audio: true })
        .then(function (stream) {
          ...
          catch (e) { try { VR.rec = new MediaRecorder(stream); } catch (e0) { VR.rec = null; } }
          /* R11 防漏释放①：录音组件初始化失败必须回收已拿到的音轨，否则麦克风被本页自己占住，
             后续任何页面（含本页重录）getUserMedia 都会报 NotReadableError「被占用」。 */
          if (!VR.rec) { releaseRecStream(); setRecUI(false); toast('录音组件初始化失败，请重试'); if (cb) cb(null); return; }
          VR.chunks = [];
          VR.start = Date.now();
          VR.armed = true;
          ...
          try { VR.rec.start(); }
          catch (e1) { VR.rec = null; releaseRecStream(); setRecUI(false); toast('无法开始录音，请检查麦克风权限'); if (cb) cb(null); return; }
          setRecUI(true);
        ...
        .catch(function (err) {
          ...
          /* 2026-09-21 麦克风排障打点 + 细分报错（原catch不分类型一律「请检查权限」，误导排障） */
          ...
          if (nm === 'NotReadableError' || nm === 'TrackStartError') {
            /* 占用自愈：释放可能残留的原生录音/识别器 + 释放本页旧流，让用户再按一次即可。
               优先 releaseMic()（静默硬释放，不会回调出一条误发的语音）；旧包没有该方法时退回
               stopVoiceRecord()，并先置 discardNative 丢弃即将到来的旧回调，同样避免误发语音。 */
            try {
              if (window.XTAppBridge && typeof window.XTAppBridge.releaseMic === 'function') {
                ...
                VR.discardNative = true;
                window.XTAppBridge.stopVoiceRecord();
              }
            } catch (e1) { /* 忽略 */ }
            try { if (VR.stream) { VR.stream.getTracks().forEach(function (t) { try { t.stop(); } catch (e2) { /* 忽略 */ } }); } } catch (e3) { /* 忽略 */ }
            VR.stream = null;
            /* R11：与 assets/ai-page.js 同口径——硬释放后自动重试一次。
               首次失败多见于上一轮原生录音/识别残留占用，释放后通常立刻可用；
               再失败才提示用户。isRetry 保证只重试一次。 */
            if (!isRetry && cb) {
              toast('正在释放麦克风，请稍候…');
              setTimeout(function () { recBegin(cb, true); }, 350);
              return;
            }
            toast('麦克风被占用，请关闭其它录音程序后重试');
          } else if (nm === 'NotAllowedError' || nm === 'PermissionDeniedError' || nm === 'SecurityError') {
            releaseRecStream();
            toast('麦克风权限被拒绝，请在系统设置-应用-星途-权限中允许');
          } else {
            /* R11：非「占用」类失败同样要回收音轨，否则本页自己把麦克风焊死。 */
            releaseRecStream();
            toast('无法访问麦克风，请检查权限');
          }
          if (cb) cb(null);
  ...
  function recFinish(action) {
    ...
    /* R11：走到这里说明没有可 stop 的 recorder（初始化失败/已停过），必须兜底回收音轨。 */
    releaseRecStream();
    setRecUI(false);
    if (action === 'cancel') toast('已取消');
    else if (action === 'tooshort') toast('说话时间太短');
  ...
  function uploadVoice(blob, dur, extOverride) {
    /* R3b-C U9：语音发送携带引用（发送前快照 + 清空草稿）。 */
    var replyRef = imReplyPayload();
    imClearReplyDraft();
    /* R3b-C U3：乐观 pending 气泡（稳定 uid），上传 / 发送失败 → 失败态 + 点重发复用原 Blob。 */
    var uid = genMsgId();
    var op = { id: uid, senderId: S.myId, content: '', kind: 'voice', time: Date.now(), duration: dur, pendingSend: true };
    if (replyRef) op.reply = replyRef;
    S.msgs.push(op);
    renderMsgs();
    ...
    var doUpload = function () {
      ...
      fetch(apiBase() + '/api/uploads/voice', {
        ...
        .then(function (res) {
          if (!res.ok || !res.d || !res.d.url) {
            imMarkMsgFailed(uid, doUpload, { file: blob });
            toast((res.d && res.d.detail) || '语音上传失败，点消息可重发');
            return;
          }
          postVoiceMsg(res.d.url, dur, replyRef, uid, blob);
        })
        .catch(function (e) {
          imMarkMsgFailed(uid, doUpload, { file: blob });
          toast('语音上传失败：' + (e.message || '网络错误') + '，点可重发');
        });
    };
    doUpload();
  }
  ...
  function postVoiceMsg(url, dur, replyRef, localMsgId, blob) {
    /* R3b-C U3：乐观 pending 气泡已由 uploadVoice 建好（localMsgId）；此处不再新建气泡，
       改为对该气泡填 url（用 FileReader 生成 dataURL 供本地即时播放，老逻辑不读 dataURL，仅存 url）。 */
    var list = S.msgs || [];
    for (var i = 0; i < list.length; i++) {
      if (String(list[i].id) === String(localMsgId)) { list[i].content = url; list[i].pendingSend = false; break; }
    }
    renderMsgs();
    /* R130：请求体补传 duration（秒，服务端 1–600 夹取，缺省/非法 → null）——
       此前只传 {content, kind}，服务端存 null，重拉后秒数丢失（本次 bug 根因）。
       R3b-C U9：补传 reply 快照。 */
    var body = { content: url, kind: 'voice', duration: imVoiceDuration(dur) };
    if (replyRef) body.reply = replyRef;
    var _peerServerId = S.peer.serverId;
    var doSend = function () {
      return fetch(apiBase() + '/api/chat/' + _peerServerId + '/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + getToken() },
        body: JSON.stringify(body)
      })
        ...
        .then(function (m) {
          ...
          if (m && m.id) {
            ...
            var rec = { id: m.id, senderId: m.senderId, content: m.content, kind: m.kind, time: new Date(m.createdAt).getTime(), server: true, read: !!m.read, duration: (svrDur != null ? svrDur : imVoiceDuration(dur)) };
            if (m.reply) rec.reply = m.reply; else if (replyRef) rec.reply = replyRef;
            S.msgs.push(rec);
            renderMsgs();
          }
          ...
          imMarkMsgFailed(localMsgId, function () {
            if (blob) {
              ...
              fetch(apiBase() + '/api/uploads/voice', {
                ...
                .then(function (r) { return r.json().then(function (d) { return { ok: r.ok, d: d }; }); })
                .then(function (res) {
                  if (!res.ok || !res.d || !res.d.url) { toast('语音上传失败，请重试'); return; }
                  postVoiceMsg(res.d.url, dur, replyRef, localMsgId, blob);
                })
                .catch(function () { toast('语音上传失败，请重试'); });
            } else { doSend(); }
          }, blob ? { file: blob } : null);
          toast('语音发送失败：' + (e && e.message ? e.message : '网络错误') + '，点可重发');
        });
    };
    doSend();
  }
  ...
  window.imSendText = function () {
    ...
    if (!S.peer) return;
    inp.value = '';
    /* R3b-C U9：发送前快照引用草稿（发送成功 / 失败都要清空草稿；引用块已随气泡渲染）。 */
    var replyRef = imReplyPayload();
    imClearReplyDraft();
    ...
    if (replyRef) msg.reply = replyRef;
    S.msgs.push(msg);
    ...
    if (S.peer.isServer) {
      ...
      var _peerServerId = S.peer.serverId;
      if (token) {
        /* R3b-C U3：把「发送」抽成可重入闭包 —— 首次发送失败时挂到失败态，点「重发」按原内容重走。 */
        var doSend = function () {
          var body = { content: text, kind: 'text' };
          if (replyRef) body.reply = replyRef;
          fetch(apiBase() + '/api/chat/' + _peerServerId + '/messages', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
            body: JSON.stringify(body)
          })
          ...
          .then(function (m) {
            // 发送成功：清除失败态 + 摘掉失败气泡，用服务端消息顶替
            imClearMsgFailed(uid);
            imDropMsg(uid);
            if (m && m.id) {
              var rec = { id: m.id, senderId: m.senderId, content: m.content, kind: m.kind, time: new Date(m.createdAt).getTime(), server: true, read: !!m.read };
              if (m.reply) rec.reply = m.reply; else if (replyRef) rec.reply = replyRef;
          ...
          .catch(function (e) {
            imMarkMsgFailed(uid, doSend, null);
            toast('发送失败：' + (e && e.message ? e.message : '网络错误') + '，点消息可重发');
          });
        };
        doSend();
      } else {
        // 有会话但无 token（离线 / 未登录）：置失败态；点重发再判 token，仍无则提示。
        imMarkMsgFailed(uid, function () {
          if (!getToken()) { toast('仍未登录，无法发送'); return; }
          // 拿到 token 后重走同一条发送闭包
          S.peer = S.peer || null;
          var body = { content: text, kind: 'text' };
          if (replyRef) body.reply = replyRef;
          ...
          .then(function (r) { if (!r.ok) throw new Error('HTTP ' + r.status); return r.json(); })
          .then(function (m) {
            imClearMsgFailed(uid); imDropMsg(uid);
            if (m && m.id) { S.msgs.push({ id: m.id, senderId: m.senderId, content: m.content, kind: m.kind, time: new Date(m.createdAt).getTime(), server: true, read: !!m.read }); }
            renderMsgs(); fetchPeerMsgs(true);
          })
          .catch(function () { toast('发送失败，请稍后重试'); });
        }, null);
      }
      return;
    }
  ...
  window.imSendLocation = function (loc) {
    ...
    var now = Date.now();
    var uid = genMsgId();
    /* R3b-C U9：位置消息同样可携带引用（发送前快照草稿并清空）。 */
    var replyRef = imReplyPayload();
    imClearReplyDraft();
    ...
    if (S.peer.isServer) {
      ...
      var _peerServerId = S.peer.serverId;
      if (token) {
        /* R3b-C U3：位置发送失败 → 失败态 + 点重发（按原坐标重走）。 */
        var doSendLoc = function () {
          ...
          if (replyRef) payload.reply = replyRef;
          fetch(apiBase() + '/api/chat/' + _peerServerId + '/messages', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
          ...
          .catch(function (e) {
            imMarkMsgFailed(uid, doSendLoc, null);
            toast('位置发送失败：' + (e && e.message ? e.message : '网络错误') + '，点可重发');
          });
        };
        doSendLoc();
      } else {
        imMarkMsgFailed(uid, function () { toast('仍未登录，无法发送位置'); }, null);
      }
      return;
    }
  ...
  function imSendGroupText(text) {
    ...
    /* R3b-C U9：群聊发送同样携带引用（发送前快照 + 清空草稿）。 */
    var replyRef = imReplyPayload();
    imClearReplyDraft();
    /* R3b-C U3：群聊此前无乐观气泡 → 失败无法呈现「重发」。为对齐微信式失败态，
       发送前先行乐观落一条 pending 气泡（带 uid），失败时置失败态、成功时用服务端消息顶替。 */
    var uid = genMsgId();
    var optimistic = { id: uid, senderId: S.myId, content: text, kind: 'text', time: Date.now(), pendingSend: true };
    if (replyRef) optimistic.reply = replyRef;
    S.msgs.push(optimistic);
    renderMsgs();

    var gid = S.group.id;
    var _doSendGroup = function () {
      var body = { content: text, kind: 'text' };
      if (replyRef) body.reply = replyRef;
      return fetch(apiBase() + '/api/groups/' + gid + '/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
        body: JSON.stringify(body)
      })
      ...
      .then(function (m) {
        imClearMsgFailed(uid); imDropMsg(uid);
        if (m && m.id) {
          var rec = { id: m.id, senderId: m.senderId, content: m.content, kind: m.kind, time: new Date(m.createdAt).getTime(), server: true };
          if (m.reply) rec.reply = m.reply; else if (replyRef) rec.reply = replyRef;
          S.msgs.push(rec);
          renderMsgs();
        }
      ...
      .catch(function (e) {
        // 失败：把乐观气泡转成失败态（保留正文），点「重发」按原内容重走
        var list = S.msgs || [];
        for (var i = 0; i < list.length; i++) { if (String(list[i].id) === String(uid)) { list[i].pendingSend = false; break; } }
        imMarkMsgFailed(uid, _doSendGroup, null);
        toast('发送失败：' + (e && e.message ? e.message : '网络错误') + '，点消息可重发');
      });
    };
    _doSendGroup();
  ...
  function imSendGroupLocation(t, sub, lat, lng, hasGeo, precise) {
    ...
    if (replyRef) payload.reply = replyRef;
    var gid = S.group.id;
    /* R3b-C U3：乐观 pending 气泡，失败置失败态（保留内容，点重发重走）。 */
    var uid = genMsgId();
    var opLoc = { id: uid, senderId: S.myId, content: t, kind: 'location', time: Date.now(), pendingSend: true };
    if (sub) opLoc.sub = sub;
    if (hasGeo) { opLoc.lat = lat; opLoc.lng = lng; }
    if (precise) opLoc.precise = true;
    if (replyRef) opLoc.reply = replyRef;
    S.msgs.push(opLoc);
    renderMsgs();

    var _doSendGroupLoc = function () {
      return fetch(apiBase() + '/api/groups/' + gid + '/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
        body: JSON.stringify(payload)
      ...
      .catch(function (e) {
        var list = S.msgs || [];
        for (var i = 0; i < list.length; i++) { if (String(list[i].id) === String(uid)) { list[i].pendingSend = false; break; } }
        imMarkMsgFailed(uid, _doSendGroupLoc, null);
        toast('位置发送失败：' + (e && e.message ? e.message : '网络错误') + '，点可重发');
      });
    };
    _doSendGroupLoc();
  ...
  // R5（2026-09-20）：300px 小浮层 → 微信式全宽底部弹出面板；收藏改长按切换；新增「自定义」tab（图片 sticker）。
  ...
  var EMOJI_LAYER_ID = 'imEmojiPanel';     // 返回键压栈 id（layer-stack）

  // 统一提示：优先 xtToast（R4-A 收口），缺失回落本页 toast，再兜底静默。
  function emojiToast(state, msg) {
    try {
      if (typeof window.xtToast === 'function') { window.xtToast(state, msg); return; }
    } catch (e) { /* 降级 */ }
    try { if (typeof toast === 'function') toast(msg); } catch (e2) { /* 静默 */ }
  }
  ...
  // 读取自定义表情数组（string[] dataURL，最多 EMOJI_CUSTOM_MAX）。
  function emojiCustomArr() {
    try {
      var v = JSON.parse(localStorage.getItem(EMOJI_CUSTOM_KEY) || '[]');
      if (!Array.isArray(v)) return [];
      return v.filter(function (s) { return typeof s === 'string' && s.indexOf('data:image/') === 0; }).slice(0, EMOJI_CUSTOM_MAX);
    } catch (e) { return []; }
  }

  // 写自定义表情；配额异常 catch 并 toast（不抛）。
  function emojiSaveCustom(arr) {
    try {
      localStorage.setItem(EMOJI_CUSTOM_KEY, JSON.stringify(arr));
      return true;
    } catch (e) {
      emojiToast('error', '存储空间不足，保存表情失败');
      return false;
    }
  }
  ...
  function emojiCurrentTab() {
    ...
    if (t === '最近') return 'recent';
    if (t === '收藏') return 'fav';
    if (t === '自定义') return 'custom';
    return 'all';
  }

  // 重绘当前 tab（仅在面板打开时刷新）。
  function emojiRefreshIfOpen() {
    var panel = $id('imEmojiPanel');
    if (panel && panel.classList.contains('open')) renderEmojiPanel(emojiCurrentTab());
  }

  // —— 面板开关（对齐同页 .im-plus-mask/.im-plus-menu 的 open 类 + 遮罩） ——
  // R6：面板升降后消息区保持底部锚定（即时 + 过渡结束各一次）
  function emojiKeepBottom() {
    var b = $id('imMsgs');
    if (!b) return;
    try { b.scrollTop = b.scrollHeight; } catch (e) { /* 静默 */ }
    setTimeout(function () { try { b.scrollTop = b.scrollHeight; } catch (e2) { /* 静默 */ } }, 260);
  }
  function imOpenEmojiPanel(tab) {
    var panel = $id('imEmojiPanel');
  ...
  function imCloseEmojiPanel() {
    ...
    try { if (window.xtLayerPop) window.xtLayerPop(EMOJI_LAYER_ID); } catch (e) { /* 静默 */ }
  }
  window.imOpenEmojiPanel = imOpenEmojiPanel;
  window.imCloseEmojiPanel = imCloseEmojiPanel;

  window.imToggleEmoji = function () {
    var panel = $id('imEmojiPanel');
    if (!panel) return;
    if (panel.classList.contains('open')) imCloseEmojiPanel();
    else imOpenEmojiPanel('recent');
  };

  window.imEmojiTab = function (tab) {
    renderEmojiPanel(tab || 'all');
  };
    ...
    if (t === 'custom') {
      ...
      if (codes.length === 0) {
        ...
        grid = codes.map(function (code) {
          ...
          if (!e) return '';
          var fav = favSet.indexOf(code) >= 0;
          // 长按 → 收藏切换；点按 → 插入文本（两者都由事件委托统一绑定）。
          return '<div class="im-emoji-item" data-code="' + esc(code) + '" title="' + esc(e.name) + '（长按收藏）">' + e.char +
            '<span class="im-emoji-fav' + (fav ? ' on' : '') + '">🌟</span></div>';
        }).join('');
      }
    }
    panel.innerHTML = '<div class="im-emoji-handle"></div>' +
      '<div class="im-emoji-tabs">' +
      '<span class="im-emoji-tab' + (t === 'recent' ? ' on' : '') + '" onclick="window.imEmojiTab(\'recent\')">最近</span>' +
      '<span class="im-emoji-tab' + (t === 'all' ? ' on' : '') + '" onclick="window.imEmojiTab(\'all\')">全部</span>' +
      '<span class="im-emoji-tab' + (t === 'fav' ? ' on' : '') + '" onclick="window.imEmojiTab(\'fav\')">收藏</span>' +
      '<span class="im-emoji-tab' + (t === 'custom' ? ' on' : '') + '" onclick="window.imEmojiTab(\'custom\')">自定义</span>' +
  ...
  function emojiRenderCustomGrid() {
    var arr = emojiCustomArr();
    var html = arr.map(function (src, i) {
      return '<div class="im-emoji-item" data-sticker="' + i + '" title="点按发送 · 长按管理"><img src="' + esc(src) + '" alt="自定义表情"></div>';
    }).join('');
    if (arr.length < EMOJI_CUSTOM_MAX) {
      html += '<div class="im-emoji-item im-emoji-add" id="imEmojiAdd" title="添加自定义表情">＋</div>';
    }
    if (arr.length === 0) {
      html = '<div class="im-emoji-empty">点「＋」从相册添加自定义表情（最多 ' + EMOJI_CUSTOM_MAX + ' 张）</div>' + html;
    }
    return html;
  }

  // 事件委托：点按 → 发送/插入；长按(≈500ms) → 收藏切换 / sticker 菜单。
  function emojiBindGrid(panel) {
    var grid = panel.querySelector('.im-emoji-grid');
    if (!grid) return;
    var pressTimer = null;
    var longFired = false;
    var startX = 0, startY = 0;
    var item = null;
    /* R143-3（自定义表情加不了的根因）：移动端一次点按 = touchend 之后浏览器再补发
       合成 mousedown/mouseup（事件委托 touchend+mouseup 双绑都会命中 end）→
       「＋」格的 imStickerPick 被连发两次 → 第二次 inp.click() 让 App 的
       onShowFileChooser 先回调 onReceiveValue(null) 取消第一次、再拉起第二次
       Activity，系统在极短间隔内二次 startActivityForResult 会被吞 →
       **选图器根本弹不出来**，表现即「自定义表情不能添加」。
       修法：记录最近一次触屏时间，800ms 内的合成 mouse 事件一律忽略
       （jsdom 走 mousedown 长按路径不受影响：无 touch → lastTouchAt=0）。 */
    var lastTouchAt = 0;
    function isGhostMouse(e) {
      return String((e && e.type) || '').indexOf('mouse') === 0 && (Date.now() - lastTouchAt) < 800;
    }
    ...
    function target(e) {
      var el = e.target;
      while (el && el !== grid) {
        if (el.classList && el.classList.contains('im-emoji-item')) return el;
        el = el.parentNode;
      }
      return null;
    }
    function begin(e) {
      if (isGhostMouse(e)) return;                 // R143-3：触屏后的合成 mouse 事件不处理
      var it = target(e);
      if (!it) return;
      item = it;
      longFired = false;
      ...
      pressTimer = setTimeout(function () {
        ...
        if (it.getAttribute('data-sticker') !== null && it.getAttribute('data-sticker') !== undefined) {
          ...
          if (code) window.imFavEmoji(code, true);
        }
      }, 500);
    }
    function move(e) {
      ...
      var dx = Math.abs((pt.clientX || 0) - startX);
      var dy = Math.abs((pt.clientY || 0) - startY);
      if (dx > 12 || dy > 12) clearPress(); // 移动超阈值 → 视为滚动，取消长按
    }
    function end(e) {
      if (String((e && e.type) || '') === 'touchend') { lastTouchAt = Date.now(); } // R143-3
      if (isGhostMouse(e)) return;                 // R143-3：防 imStickerPick/imPickEmoji 双触发
      clearPress();
      if (longFired) { // 长按已触发 → 阻止后续 click
        if (e && e.cancelable) { try { e.preventDefault(); } catch (er) { /* 静默 */ } }
        longFired = false;
        return;
      }
      var it = target(e);
      ...
      var code = it.getAttribute('data-code');
      if (code) window.imPickEmoji(code);
      if (e && e.cancelable) { try { e.preventDefault(); } catch (er) { /* 防止穿透到遮罩 */ } }
    }

    grid.addEventListener('touchstart', begin, { passive: true });
    grid.addEventListener('touchmove', move, { passive: true });
    grid.addEventListener('touchend', end);
    grid.addEventListener('touchcancel', function () { clearPress(); longFired = false; });
    grid.addEventListener('mousedown', begin);
    grid.addEventListener('mousemove', move);
    grid.addEventListener('mouseup', end);
    // 桌面兜底：右键（contextmenu）→ 收藏/菜单
  ...
  // 插入文本表情 + 记录最近使用。（保持既有签名：imPickEmoji(code)）
  window.imPickEmoji = function (code) {
    var inp = $id('imInput');
    if (inp) inp.value = (inp.value || '') + '[emoji:' + code + ']';
    var recent = emojiArr(EMOJI_RECENT_KEY, 24).filter(function (c) { return c !== code; });
    recent.unshift(code);
    try { localStorage.setItem(EMOJI_RECENT_KEY, JSON.stringify(recent.slice(0, 24))); } catch (e) { }
    emojiRefreshIfOpen();
  };

  // 切换收藏（silent=true 时不弹 toast；toastAt=true 时反馈文案）。
  window.imFavEmoji = function (code, withToast) {
    var fav = emojiArr(EMOJI_FAV_KEY, 24);
    var idx = fav.indexOf(code);
    var added;
    if (idx >= 0) { fav.splice(idx, 1); added = false; }
    else { fav.unshift(code); added = true; }
    try { localStorage.setItem(EMOJI_FAV_KEY, JSON.stringify(fav.slice(0, 24))); } catch (e) { }
    if (withToast) emojiToast('success', added ? '已收藏' : '已取消收藏');
    emojiRefreshIfOpen();
  };

  // —— 自定义表情：选图 → 压缩 → 存 localStorage ——
  window.imStickerPick = function () {
    var inp = $id('imStickerInput');
    if (!inp) { emojiToast('error', '当前页面不支持添加表情'); return; }
    try { inp.value = ''; } catch (e) { /* 老 WebView 重置失败不影响 */ }
    inp.click();
  };

  // change 事件入口（HTML：onchange="imAddSticker(this)"）
  window.imAddSticker = function (input) {
    if (!input || !input.files || !input.files[0]) return;
    var file = input.files[0];
    try { input.value = ''; } catch (e) { /* 同上 */ }
    if (!/^image\//.test(file.type || '')) { emojiToast('error', '请选择图片文件'); return; }
    if (emojiCustomArr().length >= EMOJI_CUSTOM_MAX) {
      emojiToast('warning', '自定义表情已满（最多 ' + EMOJI_CUSTOM_MAX + ' 张）');
      return;
    }
    emojiCompressImage(file, function (dataUrl, err) {
      if (!dataUrl) { emojiToast('error', err || '表情处理失败'); return; }
      var arr = emojiCustomArr();
      ...
      emojiToast('success', '已添加自定义表情');
      renderEmojiPanel('custom'); // 面板若开着，切到自定义 tab 展示新格
    });
  };
  ...
  // 估算 dataURL 的二进制字节数（base64 → bytes）。
  function emojiDataUrlBytes(dataUrl) {
    try {
      var i = dataUrl.indexOf(',');
      if (i < 0) return 0;
      var b64 = dataUrl.slice(i + 1);
      var pad = 0;
      if (b64.slice(-2) === '==') pad = 2; else if (b64.slice(-1) === '=') pad = 1;
  ...
  window.imSendSticker = function (idx) {
    ...
    try {
      ...
      var fname = 'sticker_' + Date.now() + (mime.indexOf('jpeg') >= 0 ? '.jpg' : '.png');
      var f = null;
      try { f = new File([blob], fname, { type: mime }); } catch (e3) { f = null; }
      if (!f) { // 老内核无 File 构造：给 blob 补 name 供 FormData 使用
        try { blob.name = fname; } catch (e4) { /* 只读时忽略 */ }
        f = blob;
      }
      if (typeof window.imSendImageFile === 'function') { window.imSendImageFile(f); imCloseEmojiPanel(); return; }
      emojiToast('error', '当前页面不支持发送图片');
    } catch (e) {
  ...
  function emojiOpenStickerMenu(idx, anchor) {
    ...
    var left = Math.round(r.left + r.width / 2 - mw / 2);
    var top = Math.round(r.top - mh - 6);
    if (left < 8) left = 8;
    if (left + mw > (window.innerWidth || 360) - 8) left = (window.innerWidth || 360) - 8 - mw;
  ...
  function imLiveRender(st) {
    ...
    if (mine) {
      ...
      /* R3·L2：微信式「点进即加入」—— 主按钮默认带 join=1（进入地图页自动 start 并上报自己的坐标），
         另给一个次级「仅查看」入口（join=0，只看不加入）。默认必须是「加入」，否则接收方永远不上屏。 */
      bn.innerHTML = '<span class="im-live-t">' + esc(t) + '</span>' +
        '<button type="button" class="im-live-btn" onclick="imLiveGoView()">查看</button>' +
        '<button type="button" class="im-live-btn im-live-btn--ghost" onclick="imLiveGoViewOnly()">仅查看</button>';
    }
  ...
  function imLiveViewUrl(st, opts) {
    var join = !(opts && opts.viewOnly === true);   // 默认加入（微信式点进即加入）；仅查看需显式 viewOnly
    var q = [];
    if (st && st.shareId) q.push('shareId=' + encodeURIComponent(String(st.shareId)));
    q.push('join=' + (join ? '1' : '0'));
  ...
  /* R3·L2：默认「加入」入口（join=1）—— 接收方/群成员点一次即 join 共享，双方都上屏。 */
  window.imLiveGoView = function () { location.href = imLiveViewUrl(_liveState, { viewOnly: false }); };
  /* R3·L2：次级「仅查看」入口（join=0）—— 只看对方坐标，不上报自己（保留原单向围观行为）。 */
  window.imLiveGoViewOnly = function () { location.href = imLiveViewUrl(_liveState, { viewOnly: true }); };
  ...
  /* 支持通过 ?uid= 直接进入与某位服务器好友的会话（A3/A5 深链：好友申请页「发消息」）
     【R3b-D / U7】同时支持 ?gid= 直接进入指定群聊（群消息通知点击深链，与私聊体验对齐）。 */
  function maybeAutoOpenChat() {
    var uid = 0, name = '', gid = 0;
    try {
      var p = new URLSearchParams(location.search);
      uid = Number(p.get('uid') || 0);
      name = p.get('name') || '';
      gid = Number(p.get('gid') || 0);
    } catch (e) { }
    // 群聊深链优先（通知点击直达群）
    if (gid > 0) {
      if (typeof window.imOpenGroup === 'function') window.imOpenGroup(gid);
      return;
    }
    if (!uid) return;
    var fid = 10000 + uid;
  ...
  /* 【R3b-D / U7】原生群消息通知点击的直达入口（MainActivity 深链调用 / jsdom 校验引用）：
     已在私聊页 → window.imOpenGroup 直接开会话；其它页面不存在该函数时回落站内深链
     私聊.html?gid=（与私聊 gotoChatWith 的 ?uid= 回落完全对齐）。 */
  window.gotoChatWithGroup = function (groupId, name) {
    var gid = Number(groupId);
    if (!gid) { if (typeof window.gotoChat === 'function') window.gotoChat(); return; }
    if (typeof window.imOpenGroup === 'function') {
      try { window.imOpenGroup(gid); return; } catch (e) { /* 落到深链 */ }
    }
    location.href = '私聊.html?gid=' + gid + '&name=' + encodeURIComponent(name || '');
  };
  ...
  function boot() {
    ...
    style.textContent = '@keyframes typing{0%,60%,100%{opacity:.3}30%{opacity:1}}' +
      ...
      /* R3·L2：次级「仅查看」按钮（灰描边，弱于主按钮「查看」= 加入共享） */
      '.im-live-btn--ghost{border-color:var(--border,#ddd);color:var(--text-secondary,#8a8f99)}' +
      '.im-live-btn--ghost:active{background:var(--bg,#F5F7FA)}' +
    ...
    /* 2026-09-21 麦克风防泄漏：跳页/切后台时释放本页录音流与原生残留，
       ...
       R11 补强：① 原生侧优先 releaseMic()（硬释放，含销毁 SpeechRecognizer 与 release MediaRecorder）；
       stopVoiceRecord() 在未处于录音态时是 no-op，无法销毁识别器，只能作旧包兜底。
       ② 除 pagehide 外再监听 visibilitychange（切到后台未必触发 pagehide），二者共用同一处理器。 */
    var releaseMicAll = function () {
      releaseRecStream();
      try { if (VR.rec && VR.rec.state === 'recording') { VR.rec.stop(); } } catch (e3) { }
      try {
        if (window.XTAppBridge && typeof window.XTAppBridge.releaseMic === 'function') window.XTAppBridge.releaseMic();
        else if (window.XTAppBridge && typeof window.XTAppBridge.stopVoiceRecord === 'function') window.XTAppBridge.stopVoiceRecord();
      } catch (e4) { }
    };
    window.addEventListener('pagehide', releaseMicAll);
    document.addEventListener('visibilitychange', function () {
      if (document.visibilityState === 'hidden') releaseMicAll();
    });
    ...
    if (inp) {
      ...
      /* R141（2026-09-21）：点聊天输入框 → 自动收起表情面板 / 加号菜单（微信式互斥）。
         修复「点开表情菜单后再点聊天对话框，面板不自动收起」：此前输入框只绑了
         keydown / paste，两者都不关面板；而表情遮罩 .im-emoji-mask 自 R5 起已
         display:none（微信式无遮罩），点输入框不会命中任何收起逻辑，导致面板与
         键盘同时打开、消息区被挤占，必须再点一次表情键才能关。
         幂等安全：imCloseEmojiPanel / imClosePlusMenu 只做 classList.remove('open')
         + 底部锚定重排；未打开时调用无副作用（layer-stack 的 xtLayerPop 在内部栈
      ...
      /* R143-2：补 touchstart —— 个别老 WebView 点输入框既不触发 focus 也不触发 click，
         touchstart 是最原始可靠的信号，保证「面板开着 → 点输入框」必收面板。 */
      inp.addEventListener('touchstart', imCloseFloatPanels, { passive: true });
    }
  ...
  /* ==================== R3-L1 问题三：手机端键盘避让 ====================
     背景：顶层 Window 已 edge-to-edge（setDecorFitsSystemWindows(false)），
     Android 下 IME inset 不再可靠自动 resize（adjustResize 失效）。
  ...
  function imKbApply() {
    try {
      ...
      var visualH = vv.height || layoutH;         // 可视视口高（键盘弹起时变小）
      var kb = layoutH - visualH;                 // 键盘占用高度（近似）
      if (kb > 60) {
        /* 键盘弹起：聊天容器缩到可视区高，输入栏贴键盘上沿 */
        conv.style.height = visualH + 'px';
        conv.style.paddingBottom = '0px';
      } else {
        /* 收键盘：恢复自然高度，仅保留底部安全区 */
        conv.style.height = '';
        conv.style.paddingBottom = '';
      }
    } catch (e2) { /* 避让失败：切回默认布局，不影响功能 */ }
  }
  function imKbBind() {
    if (_kbBound) return;
    _kbBound = true;
    try {
      ...
      if (vv && vv.addEventListener) {
        vv.addEventListener('resize', imKbApply);
        vv.addEventListener('scroll', imKbApply);
      }
      /* 无 visualViewport 的老机型：储备 window resize 也调一次（可能不精确，但不会比不处理更差） */
      window.addEventListener('resize', imKbApply);
    } catch (e) { /* 绑定失败：仅依靠 meta 方案 */ }
  }
  /* 进出会话时调用：进入则绑定监听+立即算一次；退出则清零避让样式 */
  function imKbOnEnter() { try { imKbBind(); imKbApply(); } catch (e) { } }
  function imKbOnLeave() {
    try {
      var conv = document.getElementById('imConv');
      if (conv) { conv.style.paddingBottom = ''; conv.style.height = ''; }
    } catch (e) { }
  }
  ...
  window.__IM_TEST__ = {
    ...
    loadChats: loadChats,
    /* R5（2026-09-20）：表情面板校验钩子（仅测试引用，零运行时行为影响）。 */
    renderContent: renderContent,
    renderEmojiPanel: renderEmojiPanel,
    emojiCustomArr: emojiCustomArr,
    emojiDataUrlBytes: emojiDataUrlBytes,
    SWIPE_PX: SWIPE_PX,
    ...
    /* R3b-C U9/U3（2026-09-20）：长按引用 / 发送失败重发 校验钩子（仅测试引用，零运行时行为影响）。
       jsdom 可直接注入 S.msgs / S.peer 后驱动 imShowMsgMenu / imStartReply / imSendText / imRetryMsg。 */
    imShowMsgMenu: window.imShowMsgMenu,
    imStartReply: window.imStartReply,
    imClearReplyDraft: window.imClearReplyDraft,
    imJumpToMsg: window.imJumpToMsg,
    imMenuQuote: window.imMenuQuote,
    imRetryMsg: window.imRetryMsg,
    imReplySummary: imReplySummary,
    imReplySenderName: imReplySenderName,
    imReplyBlockHtml: imReplyBlockHtml,
    imReplyPayload: imReplyPayload,
    imRenderQuoteBar: imRenderQuoteBar,
    imMarkMsgFailed: imMarkMsgFailed,
    imClearMsgFailed: imClearMsgFailed,
    imSendGroupText: imSendGroupText,
    getReplyDraft: function () { return S.replyDraft; },
    isFailed: function (id) { return !!_imFailed[String(id)]; },
    setMyId: function (v) { S.myId = v; },
    setPeer: function (p) { S.peer = p; },
    setGroup: function (g) { S.group = g; },
    setMsgs: function (arr) { S.msgs = arr || []; },
    getMsgs: function () { return S.msgs; },
    /* R3b-D U7（2026-09-20）：群消息通知点击直达群聊 校验钩子（仅测试引用，零运行时行为影响） */
    gotoChatWithGroup: window.gotoChatWithGroup,
    maybeAutoOpenChat: maybeAutoOpenChat
  };

...
(function () {
  ...
  // C8（2026-09-22）：防闪烁——首次扫描前 [data-icon] 空壳靠 common.css 的
  // html:not(.icons-ready) [data-icon]{visibility:hidden} 隐藏（保持占位、不跳动）；
  // 本函数跑完（无论成功/异常）在 finally 里给 documentElement 加 icons-ready，
  // 使图标显形。finally 保证「绝不永久隐藏」——即使渲染中途抛错，class 也会加上。
  var _iconWarned = {}; // 未知图标名只 warn 一次，避免刷屏
  function _markIconsReady() {
    try {
      if (typeof document !== "undefined" && document.documentElement) {
        document.documentElement.className += " icons-ready";
      }
    } catch (e) { /* 老内核兜底：置位失败也不影响图标已注入 */ }
  }
  function autoRender() {
    ...
    try {
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        if (!n) continue;
        if (!window.LUCIDE_ICONS[n]) {
          if (!_iconWarned[n]) {
            _iconWarned[n] = 1;
            if (typeof console !== "undefined" && console.warn) {
              console.warn("[icon-map] 未知图标名:", n, "（已跳过，检查 data-icon 拼写/字典）");
            }
          }
          continue;
        }

...
(function () {
  ...
  var PAGE = {
    ...
    /* R3-L4 设备统计 */
    devStats: null,          // GET /api/admin/devices/stats 结果
    devItems: [],            // GET /api/admin/devices 分页明细
    devTotal: 0,
    devPage: 1,
    devSize: 20,
    devPages: 0,
    devFilter: { brand: '', model: '', appVersion: '' },
    devLoading: false,
    devError: ''
  };
  ...
  function syncTabs() {
    ...
    var ul = $('admUserList');
    if (ul) ul.style.display = (PAGE.tab === 'users' || PAGE.tab === 'online') ? '' : 'none';
  ...
  window.admSwitchTab = function (tab) {
    ...
    else if (tab === 'devices') loadDeviceAll();
    else renderUserList();
  };
  ...
  window.admRefresh = function () {
    ...
    else if (PAGE.tab === 'devices') loadDeviceAll();
    else loadUsers();
  };

  /* ================= R3-L4 设备统计 ================= */
  // 明细行归一化：后端已 camelCase + snake_case 双写，这里统一成一套。
  function normalizeDevice(d) {
    d = d || {};
    return {
      userId: d.userId !== undefined ? d.userId : d.user_id,
      username: admStr(d.username || ''),
      nickname: admStr(d.nickname || d.username || ''),
      brand: admStr(d.brand || '—'),
      model: admStr(d.model || '—'),
      osVersion: admStr(d.osVersion || '—'),
      appVersion: admStr(d.appVersion || '—'),
      screen: admStr(d.screen || (d.screenWidth && d.screenHeight ? (d.screenWidth + 'x' + d.screenHeight) : '—')),
      language: admStr(d.language || '—'),
      androidId: admStr(d.androidId || '—'),
      lastActive: admStr(d.lastActive || d.last_active || '')
    };
  }

  function devDistHtml(title, list, emptyText) {
    list = list || [];
    var body = '';
    if (!list.length) {
      body = '<div class="adm-muted">' + admEsc(emptyText || '暂无数据') + '</div>';
    } else {
      var max = 0;
      for (var i = 0; i < list.length; i++) { max = Math.max(max, admNum(list[i] && list[i].count)); }
      if (max <= 0) max = 1;
      for (var j = 0; j < list.length; j++) {
        var it = list[j] || {};
        var cnt = admNum(it.count);
        var pct = Math.round(cnt / max * 100);
        body += '<div class="adm-dist-row">' +
          '<div class="adm-dist-name" title="' + admEsc(it.name) + '">' + admEsc(it.name || '—') + '</div>' +
          '<div class="adm-dist-bar"><span style="width:' + pct + '%"></span></div>' +
          '<div class="adm-dist-num">' + cnt + '</div>' +
        '</div>';
      }
    }
    return '<div class="adm-dist"><div class="adm-dist-title">' + admEsc(title) + '</div>' + body + '</div>';
  }

  function renderDevStats() {
    var host = $('admDeviceStats');
    if (!host) return;
    if (PAGE.devLoading && !PAGE.devStats) { host.innerHTML = '<div class="adm-empty">加载中…</div>'; return; }
    if (PAGE.devError) { host.innerHTML = '<div class="adm-empty err">' + admEsc(PAGE.devError) + '</div>'; return; }
    var s = PAGE.devStats || {};
    var num = function (v) { return (v === null || v === undefined) ? '—' : String(admNum(v)); };
    var total = ovPick(s, 'totalDevices', 'total_devices');
    var a7 = ovPick(s, 'activeUsers7d', 'active_users7d');
    var a30 = ovPick(s, 'activeUsers30d', 'active_users30d');
    if (a7 === null) a7 = s.active7d;
    if (a30 === null) a30 = s.active30d;

    var cards = '<div class="adm-overview adm-dev-ov">' +
      '<div class="adm-ov-card"><div class="adm-ov-icon"><span class="nav-icon" data-icon="smartphone" data-icon-size="18"></span></div>' +
        '<div><div class="adm-ov-num">' + num(total) + '</div><div class="adm-ov-label">设备总数</div></div></div>' +
      '<div class="adm-ov-card"><div class="adm-ov-icon"><span class="nav-icon" data-icon="zap" data-icon-size="18"></span></div>' +
        '<div><div class="adm-ov-num">' + num(a7) + '</div><div class="adm-ov-label">近7天活跃</div></div></div>' +
      '<div class="adm-ov-card"><div class="adm-ov-icon"><span class="nav-icon" data-icon="clock" data-icon-size="18"></span></div>' +
        '<div><div class="adm-ov-num">' + num(a30) + '</div><div class="adm-ov-label">近30天活跃</div></div></div>' +
      '</div>';

    var dists = '<div class="adm-devgrid">' +
      devDistHtml('品牌分布', s.brandDist, '暂无品牌数据') +
      devDistHtml('型号 Top', s.modelTop, '暂无型号数据') +
      devDistHtml('App 版本', s.appVersionDist, '暂无版本数据') +
      devDistHtml('Android 版本', s.osVersionDist, '暂无系统数据') +
      '</div>';

    host.innerHTML = cards + dists;
    try { if (window.lucideAutoRender) window.lucideAutoRender(); } catch (e) { /* 忽略 */ }
  }

  function renderDevTable() {
    var host = $('admDeviceTable');
    if (!host) return;
    if (PAGE.devLoading) { host.innerHTML = '<div class="adm-empty">加载中…</div>'; return; }
    if (PAGE.devError) { host.innerHTML = '<div class="adm-empty err">' + admEsc(PAGE.devError) + '</div>'; return; }
    var list = (PAGE.devItems || []).map(normalizeDevice);
    if (!list.length) { host.innerHTML = '<div class="adm-empty">暂无设备记录</div>'; return; }

    var html = '<div class="adm-dev-tablewrap"><table class="adm-dev-table"><thead><tr>' +
      '<th>用户</th><th>品牌</th><th>型号</th><th>系统</th><th>App</th><th>屏幕</th><th>语言</th><th>androidId</th><th>最近活跃</th>' +
      '</tr></thead><tbody>';
    for (var i = 0; i < list.length; i++) {
      var d = list[i];
      html += '<tr>' +
        '<td><b>' + admEsc(d.nickname || '未命名') + '</b><div class="adm-dev-sub">@' + admEsc(d.username || '-') + '</div></td>' +
        '<td>' + admEsc(d.brand) + '</td>' +
        '<td>' + admEsc(d.model) + '</td>' +
        '<td>' + admEsc(d.osVersion) + '</td>' +
        '<td>' + admEsc(d.appVersion) + '</td>' +
        '<td>' + admEsc(d.screen) + '</td>' +
        '<td>' + admEsc(d.language) + '</td>' +
        '<td class="adm-dev-mask">' + admEsc(d.androidId) + '</td>' +
        '<td>' + admEsc(fmtTime(d.lastActive)) + '</td>' +
      '</tr>';
    }
    html += '</tbody></table></div>';

    var pages = admNum(PAGE.devPages);
    var page = admNum(PAGE.devPage);
    var total = admNum(PAGE.devTotal);
    html += '<div class="adm-pager">' +
      '<button class="btn btn-sm btn-secondary" ' + (page <= 1 ? 'disabled' : '') + ' onclick="admDevPage(' + (page - 1) + ')">上一页</button>' +
      '<span class="adm-pager-info">第 ' + page + ' / ' + (pages || 1) + ' 页 · 共 ' + total + ' 条</span>' +
      '<button class="btn btn-sm btn-secondary" ' + (page >= pages ? 'disabled' : '') + ' onclick="admDevPage(' + (page + 1) + ')">下一页</button>' +
    '</div>';
    host.innerHTML = html;
  }

  function renderDevFilters() {
    var host = $('admDeviceFilters');
    if (!host) return;
    var f = PAGE.devFilter || { brand: '', model: '', appVersion: '' };
    host.innerHTML =
      '<input class="adm-dev-input" id="admDevFBrand" placeholder="品牌" value="' + admEsc(f.brand) + '">' +
      '<input class="adm-dev-input" id="admDevFModel" placeholder="型号" value="' + admEsc(f.model) + '">' +
      '<input class="adm-dev-input" id="admDevFApp" placeholder="App版本" value="' + admEsc(f.appVersion) + '">' +
      '<button class="btn btn-sm btn-primary" onclick="admDevSearch()">筛选</button>' +
      '<button class="btn btn-sm btn-secondary" onclick="admDevReset()">重置</button>' +
      '<button class="btn btn-sm btn-secondary" onclick="admDevExport()">导出 CSV</button>';
  }
  ...
  function devQuery() {
    var f = PAGE.devFilter || {};
    var q = [];
    if (f.brand) q.push('brand=' + encodeURIComponent(f.brand));
    if (f.model) q.push('model=' + encodeURIComponent(f.model));
    if (f.appVersion) q.push('appVersion=' + encodeURIComponent(f.appVersion));
    return q.length ? ('&' + q.join('&')) : '';
  }

  function loadDevStats() {
    return adminApi('/api/admin/devices/stats').then(function (d) {
      PAGE.devStats = d || {};
    }).catch(function (e) {
      if (e && e.isForbidden) { onForbidden(); throw e; }
      if (e && e.status === 401) { onExpired(); throw e; }
      PAGE.devError = '设备统计加载失败，请稍后刷新';
    });
  }

  function loadDevTable() {
    var url = '/api/admin/devices?page=' + PAGE.devPage + '&size=' + PAGE.devSize + devQuery();
    return adminApi(url).then(function (d) {
      PAGE.devItems = toList(d, ['items', 'data', 'list']);
      PAGE.devTotal = admNum(d && d.total);
      PAGE.devPages = admNum(d && d.pages);
      if (!PAGE.devPages && PAGE.devTotal) PAGE.devPages = Math.ceil(PAGE.devTotal / PAGE.devSize);
    }).catch(function (e) {
      if (e && e.isForbidden) { onForbidden(); throw e; }
      if (e && e.status === 401) { onExpired(); throw e; }
      PAGE.devItems = [];
      PAGE.devError = '设备明细加载失败，请稍后刷新';
    });
  }

  function loadDeviceAll() {
    PAGE.devLoading = true;
    PAGE.devError = '';
    renderDevPanel();
    var t1 = loadDevStats();
    var t2 = loadDevTable();
    // 两个请求并发；任一 403 触发跳转后不再渲染
    return Promise.all([t1.catch(function () { return null; }), t2.catch(function () { return null; })])
      .then(function () {
        PAGE.devLoading = false;
        renderDevPanel();
      });
  }

  window.admDevPage = function (p) {
    p = admNum(p);
    if (p < 1) p = 1;
    if (PAGE.devPages && p > PAGE.devPages) p = PAGE.devPages;
    PAGE.devPage = p;
    PAGE.devLoading = true;
    renderDevTable();
    loadDevTable().then(function () { PAGE.devLoading = false; renderDevTable(); });
  };

  window.admDevSearch = function () {
    var b = $('admDevFBrand'); var m = $('admDevFModel'); var a = $('admDevFApp');
    PAGE.devFilter = {
      brand: (b && b.value || '').trim(),
      model: (m && m.value || '').trim(),
      appVersion: (a && a.value || '').trim()
    };
    PAGE.devPage = 1;
    PAGE.devLoading = true;
    renderDevPanel();
    loadDevTable().then(function () { PAGE.devLoading = false; renderDevPanel(); });
  };

  window.admDevReset = function () {
    PAGE.devFilter = { brand: '', model: '', appVersion: '' };
    PAGE.devPage = 1;
    PAGE.devLoading = true;
    renderDevPanel();
    loadDevTable().then(function () { PAGE.devLoading = false; renderDevPanel(); });
  ...
  // 导出：带鉴权头直接拉 Blob，再触发下载。绝不用 <a href> 直连（会丢 Bearer 令牌）。
  window.admDevExport = function () {
    if (typeof window.apiGetToken !== 'function' || typeof window.API_BASE === 'undefined') {
      toast('页面脚本未就绪，请刷新重试', 'error');
      return;
    }
    var token = window.apiGetToken();
    var url = window.API_BASE + '/api/admin/devices/export?' + devQuery().replace(/^&/, '');
    var fname = 'devices_' + new Date().toISOString().slice(0, 19).replace(/[-:T]/g, '') + '.csv';
    toast('正在导出…', 'info');
    fetch(url, { headers: token ? { 'Authorization': 'Bearer ' + token } : {} })
      .then(function (res) {
        if (res.status === 403) { onForbidden(); throw makeErr(403, '无权限'); }
        if (res.status === 401) { onExpired(); throw makeErr(401, '登录已过期'); }
        if (!res.ok) throw makeErr(res.status, '导出失败');
        return res.blob();
      })
      .then(function (blob) {
      ...
      .catch(function (e) {
        if (e && (e.isForbidden || e.status === 401)) return; // 已跳转

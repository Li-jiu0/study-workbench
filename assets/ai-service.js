...
(function () {
    ...
    try {
      ...
      if (resp.ok) {
        ...
        for (var i = 0; i < models.length; i++) {
          // R11g：必须把 relayAvailable 一起带出来。
          // 服务端 /api/ai/models 对国内平台按「已配 Key」过滤，但对海外平台
          // （gemini / openrouter）是**恒下发**的，用 relayAvailable 表明「key+proxy 是否双齐」。
          // 原先只取 {id,name} 丢掉该字段，导致前端把「平台在列表里」误判成「通道就绪」。
          // 语义：仅显式 false 才算未就绪；字段缺失（国内平台）一律视为可用。
          list.push({
            id: models[i].id,
            name: models[i].name,
            relayAvailable: (models[i].relayAvailable === false) ? false : true
          });
        }
  ...
  // 该接口对**国内平台**按「已配置 Key」过滤（不在列表 = 未配 Key = 不可用）；
  // 对**海外平台**（gemini / openrouter）恒下发，就绪与否看 relayAvailable
  // （= key 且 proxy 双齐，或显式 *_ALLOW_DIRECT=1）。
  // ⚠️ R11g 修正：原注释写「gemini / openrouter 会被自动剔除」——与后端实现不符，
  // 后端一直是带 relayAvailable:false 下发。旧逻辑只看「是否在列表里」，
  // 于是服务端海外代理没配好时也会被判成「正常」（假阳性）。现改为两者都看。
  ...
  async function relayProbeAvailable(mc) {
    ...
    for (var i = 0; i < list.length; i++) {
      if (list[i] && String(list[i].id) === pname) {
        // 服务端明确标记「中转不可用」（海外平台缺 key 或缺代理）→ 如实判不可用
        if (list[i].relayAvailable === false) return { ok: false, err: "unavailable" };
        return { ok: true, err: "" };
      }
    }
  ...
  function aiHealthCheckImpl(modelId, cfgOverride) {
    ...
    return new Promise(function (resolve) {
      ...
      // 可用性以服务端 GET /api/ai/models 为 **权威源**：
      //   国内平台：在列表里 = 服务端已配好 Key；不在列表里 = 未配 Key，不可用。
      //   海外平台（gemini / openrouter）：恒在列表里，**须再看 relayAvailable**
      //     （R11g 修正：此前只看是否在列表，会把「服务端海外代理没配好」误报成「正常」）。
      if (eff.relay) {

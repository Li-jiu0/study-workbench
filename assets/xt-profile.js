...
(function () {
  ...
  /** 在线时读服务端真实态（邮箱绑定态 + 账号 + 账号变更锁定态）；
      失败 / 离线静默保留本地态（邮箱回落本地值、账号回落本机缓存/登录态、锁定态按可编辑展示）。 */
  function loadRemoteMe(cb) {
    var done = false;
    var finish = function () { if (done) return; done = true; if (cb) cb(); };
    try {
      if (typeof window.isOnlineSession !== 'function' || !window.isOnlineSession()) { finish(); return; }
      if (typeof window.api !== 'function') { finish(); return; }
      window.api('/api/auth/me', { method: 'GET' }).then(function (me) {
        emailRemote = (me && me.email) ? String(me.email) : '';
        emailRemoteLoaded = true;
        // R141/R142：服务端账号（account）——拉到即缓存落盘，离线也显示同一个账号。
        // ⚠️ 必须「原样」保留大小写与中文（account 即注册账号），绝不能再 toLowerCase()——
        //    否则 ID 位置显示 sage 而非注册时的 Sage，与需求「ID 位置显示注册账号」相悖。
        var acc = (me && me.account) ? String(me.account).trim() : '';
        if (acc) { accountRemote = acc; accountRemoteLoaded = true; setItem('xt_profile_account_v2', acc); }
        // 账号变更锁定态：独立请求，失败静默（不阻断邮箱/账号态刷新）
        return window.api('/api/users/me/change-locks', { method: 'GET' })['catch'](function () { return null; });
      }).then(function (lk) {
        var a = lk && lk.locks && lk.locks.account;
        if (a) {
          accountLock = {
            loaded: true,
            locked: !!a.locked,
            ...
            cooldownDays: (lk && lk.cooldownDays) || 30
          };
        }
        finish();
  ...
  /* R141/R142（2026-09-21）：账号（account）改「服务端权威 + 本机缓存 + 本机登录态」三级取值。
     优先级：
       ① 本次会话已从 /api/auth/me 拉到 accountRemote（服务端真实账号，可用于登录）；
       ② 本机缓存 xt_profile_account_v2（上次在线时落盘的服务端值 → 离线也显示同一个账号）；
       ③ 本机登录态直读 study_workbench_auth 的 account / username（**原样返回，不再哈希**）；
       ④ 游客兜底：无任何真实身份时才本地派生 6 位并持久化。
     改动前全站显示的 6 位是「各端各自算的哈希」，与服务端账号无关、也无法用于登录；
     R142 口径纠偏后，账号即「注册/登录所用的账号」，故 ③ 必须原样展示，不能再派生。 */
  var accountRemote = '';           // 服务端账号（'' = 未拉到 / 未登录）
  var accountRemoteLoaded = false;  // 是否已成功拉到服务端账号
  function userId() {
    if (accountRemoteLoaded && accountRemote) { return accountRemote; }
    var cachedAcc = getItem('xt_profile_account_v2');
    if (cachedAcc) { return cachedAcc; }
    // ③ 本机登录态：账号优先，其次注册用户名（两者都是「账号」的合法取值）
    try {
      var a = JSON.parse(localStorage.getItem('study_workbench_auth') || 'null');
      if (a && typeof a === 'object') {
        var cand = String(a.account || a.username || '').trim();
        if (cand) { return cand; }
      }
    } catch (e) { /* 忽略 */ }
    // ④ 游客兜底（无任何真实身份）：本地派生一次并持久化，绝不每次刷新变一个
    var cached = getItem('xt_profile_uid_v2');
    if (cached) { return cached; }
    var base = getItem('study_workbench_last_account') || '';
    var id = base ? derive6(base) : derive6('local:' + Date.now() + ':' + Math.random());
    setItem('xt_profile_uid_v2', id);
    return id;
  }
  /* 账号 变更锁定态（服务端 profile_change_log：30 天一次）。
     未拉到（离线 / 请求失败）时保持「可编辑」外观，提交时仍由服务端权威拦截并给出下次可改时间。 */
  var accountLock = { loaded: false, locked: false, changedAt: '', nextAvailableAt: '', cooldownDays: 30 };
  /* 账号格式校验：与服务端 schemas.check_account_format 完全一致
     （3~20 位、中文 / 字母 / 数字 / 下划线；保留原大小写，不做小写折叠）。 */
  function xtValidateAccount(v) {
    var s = String(v === null || v === undefined ? '' : v).trim();
    if (!/^[一-龥A-Za-z0-9_]{3,20}$/.test(s)) { return ''; }
    return s;
  }
  ...
  /* 入口：个人资料.html?user=<服务端数字 uid>（不带 ?user= → 本人视角，行为完全不变）。
     ...
     （nickname / avatarUrl / motto / bio / city / goal / createdAt / online /
       lastSeenAt / isFriend / stats / notes 条数 / **account（账号，R142：仅好友可拿）**）。
     以下敏感字段即使接口返回，也「绝不渲染」：
  ...
  /** TA 分组（仅公开白名单字段；空值行不渲染）。
      ...
      非好友态隐藏「系统 → 关于」（该项已于 R141 按用户要求整体删除）。
      momentCount < 0 表示条数未知 → 不显示数值。 */
  function otherGroupDefs(uid, u, momentCount, moments) {
    ...
    /* R142（2026-09-21）：显示 TA 的「账号」——用户要求「能在好友的个人资料中看到好友的账号」。
       服务端 public_profile 仅在「本人或好友」时才下发 account，故此处有值即代表可见；
       非好友 / 陌生人拿不到该字段，自然不渲染。 */
    var oacct = String((u && u.account) || '').trim();
    if (oacct) { aboutRows.push({ icon: 'user', title: '账号', val: oacct, info: true }); }
    ...
    // R141（2026-09-21）：按用户要求删除「关于」菜单，TA 视角同页同删，保持两视角一致。
    return out;
  }
  ...
  function paintOther(root, uid, u, momentCount, moments) {
    ...
    html += '<div class="xtp-tip">资料来自服务端公开信息；账号仅好友可见；性别 / 生日 / 手机号 / 邮箱等隐私字段不对外展示。</div>';
    root.innerHTML = html;
  ...
  /** 同源（或 file: 同目录）的 动态空间 / 社区 页 referrer；否则返回空串。
      R143-1（2026-09-21）：新增显式回跳参数 ?from=私聊&uid=<uid>&name=<昵称> ——
      私聊页头部「✎」进入本页（TA 视角）时带上（App WebView 内 document.referrer
      可能为空，必须用 URL 参数兜底）；返回键据此回到 私聊.html?uid= 并由其
      maybeAutoOpenChat() 直接重开该会话（用户要求：返回仍回当前聊天页面）。
      安全：只放行白名单值「私聊」，uid 在回跳前已是纯数字（viewUid 校验过的来源）。 */
  function otherReferrer() {
    try {
      var mf = /[?&]from=([^&]+)/.exec(location.search || '');
      if (mf && decodeURIComponent(mf[1]) === '私聊') {
        var mq = /[?&]uid=([^&]+)/.exec(location.search || '');
        var mn = /[?&]name=([^&]+)/.exec(location.search || '');
        if (mq && /^\d+$/.test(mq[1])) {
          var back = '私聊.html?uid=' + encodeURIComponent(mq[1]);
          if (mn && mn[1]) { back += '&name=' + mn[1]; }
          return back;
        }
      }
    } catch (eF) { /* 参数异常 → 落回 referrer 逻辑 */ }
    var ref = String(document.referrer || '');
    if (!ref) { return ''; }
    if (!/动态空间\.html|社区\.html|私聊\.html/.test(ref)) { return ''; }
  ...
  function groupDefs(m) {
    return [
      ...
      /* R141（2026-09-21）：按用户要求删除「系统 → 关于」菜单整组（该组此前只剩「关于」一项，
         空组会渲染出一个空白卡片，故整组移除）。关于页 关于.html 与 xtpAbout() 弹层函数均保留
         （保留 DOM/函数、不删原功能），仍可由其它入口访问。 */
    ];
  }
    ...
    return '<section class="xtp-hero" id="xtpHero" data-act="edit">' +
      ...
      '<div class="xtp-hero-main">' +
        ...
        '<div class="xtp-hero-id">ID：<b>' + esc(userId()) + '</b>' +
          '<span class="xtp-hero-qr" title="账号">' + QR_SVG + '</span></div>' +
        (motto ? '<div class="xtp-hero-motto">' + esc(motto) + '</div>' : '') +
      '</div>' +
  ...
  function editBody() {
    ...
    return '' +
      ...
      '<div class="xtp-sec">' +
        ...
        '</div>' +
      '</div>' +
      '<div class="xtp-tip">改动即时生效并保存到本机（localStorage）。账号 可修改（30 天一次，需联网）；学习等级 / 已绑定邮箱 只读；手机号仅本机保存、未经服务器验证。</div>';
  }

  /* ============ R141/R142（2026-09-21）：账号（account）编辑行 ============
     - 在线（已登录）：input 可编辑 + 「保存」+「复制」；服务端已判定 30 天锁定 → 只读并展示下次可修改时间；
     - 离线 / 未登录：只读展示 + 「复制」，附「登录后可修改」说明（离线改不了，服务端无法同步）。
     锁定态来自 GET /api/users/me/change-locks 的 locks.account（loadRemoteMe 时拉取，失败静默）。 */
  function accountNoteText(locked) {
    if (locked && accountLock.nextAvailableAt) {
      return '已锁定：下次可修改时间 ' + accountLock.nextAvailableAt + '（30 天一次）。';
    }
    return '即你注册 / 登录所用的账号，每 30 天仅可改一次；好友能看到它。';
  }
  function accountFieldHtml() {
    var online = (typeof window.isOnlineSession === 'function' && window.isOnlineSession());
    var cur = userId();
    if (!online) {
      return '<div class="xtp-field">' +
        '<div class="xtp-field-label">账号</div>' +
        '<div class="xtp-field-body">' +
          '<div class="xtp-readval" id="xtpEditUid">' + esc(cur) + '</div>' +
          '<div class="xtp-frow-note">登录后可修改（30 天一次）</div>' +
        '</div>' +
        '<button class="xtp-mini-btn" type="button" id="xtpEditCopyUid">复制</button>' +
      '</div>';
    }
    var locked = !!accountLock.locked;
    return '<div class="xtp-field">' +
      '<div class="xtp-field-label">账号</div>' +
      '<div class="xtp-field-body">' +
        '<input class="xtp-input" id="xtpEditUid" maxlength="20" autocomplete="off" spellcheck="false"' +
          (locked ? ' readonly' : '') +
          ' placeholder="3-20 位，中文 / 字母 / 数字 / 下划线" value="' + esc(cur) + '">' +
        '<div class="xtp-frow-note" id="xtpEditUidNote">' + esc(accountNoteText(locked)) + '</div>' +
      '</div>' +
      '<button class="xtp-mini-btn" type="button" id="xtpEditCopyUid">复制</button>' +
      (locked ? '' : '<button class="xtp-mini-btn" type="button" id="xtpEditSaveUid">保存</button>') +
    '</div>';
  }
  /* 保存账号：前端先做同口径格式校验，最终以服务端为准
     （唯一性 409 / 30 天锁定 400 的 detail 文案直接透传给用户）。 */
  function xtpSaveAccount(input, btn) {
    if (typeof window.isOnlineSession !== 'function' || !window.isOnlineSession() || typeof window.api !== 'function') {
      toast('离线模式无法修改账号，请联网后重试', true);
      return;
    }
    var v = xtValidateAccount(input && input.value);
    if (!v) {
      toast('账号需为 3-20 位，可含中文、字母、数字与下划线', true);
      return;
    }
    if (v.toLowerCase() === String(userId()).trim().toLowerCase()) { toast('账号未发生变化'); return; }
    var old = btn ? btn.textContent : '';
    if (btn) { btn.disabled = true; btn.textContent = '保存中…'; }
    window.api('/api/users/me/account', { method: 'PUT', body: { account: v } }).then(function (r) {
      accountRemote = (r && r.account) ? String(r.account) : v;
      accountRemoteLoaded = true;
      setItem('xt_profile_account_v2', accountRemote);
      var a = r && r.changeLocks && r.changeLocks.account;
      if (a) {
        accountLock = {
          loaded: true, locked: !!a.locked, changedAt: a.changedAt || '',
          nextAvailableAt: a.nextAvailableAt || '', cooldownDays: (r && r.cooldownDays) || 30
        };
      }
      renderPage();
      var ev = $('xtpView_edit');
      if (ev && ev.classList.contains('on')) { renderView('edit', ev); }
      toast('账号已更新为 ' + accountRemote + '，好友可以看到');
    })['catch'](function (err) {
      if (btn) { btn.disabled = false; btn.textContent = old; }
      toast((err && err.message) ? err.message : '账号修改失败，请稍后重试', true);
    });
  }
  function bindEditView(el) {
    ...
    if (copy) { copy.addEventListener('click', function () { copyText(userId(), '账号'); }); }
    /* R141/R142：账号保存（服务端权威校验：格式 / 唯一性 / 30 天锁定） */
    var uidSave = el.querySelector('#xtpEditSaveUid');
    var uidInput = el.querySelector('#xtpEditUid');
    if (uidSave && uidInput) {
      uidSave.addEventListener('click', function () { xtpSaveAccount(uidInput, uidSave); });
      uidInput.addEventListener('keydown', function (ev) {
        if (ev.key === 'Enter') { ev.preventDefault(); xtpSaveAccount(uidInput, uidSave); }
      });
    }
    ...
    if (ok) {
      ok.addEventListener('click', function () {
        try {
          ...
          /* 2026-09-21 头像不同步修复：此前头像只写本机 profile.avatarImg（base64），
             从不上传服务器 → 私聊/群聊/动态/好友列表全读服务器 avatarUrl（chat-local.js
             通篇只认 avatarUrl），一直显示旧头像。现在线模式下把同一张裁剪图
             POST /api/users/me/avatar（与 api.js onProfileAvatarFile 同端点），
             服务器落 uploads/avatars/ 并更新 users.avatar → 全站同步。
             离线/file:// 直开时 window.api 不存在或调用失败，静默保持仅本机。 */
          try {
            if (typeof window.api === 'function') {
              ...
              afd.append('file', new Blob([arr], { type: 'image/jpeg' }), 'avatar.jpg');
              window.api('/api/users/me/avatar', { method: 'POST', body: afd }).then(function (r) {
                try { if (window.CURRENT_USER && r && r.avatarUrl) { window.CURRENT_USER.avatarUrl = r.avatarUrl; } } catch (e2) { }
                toast('头像已更新并同步');
              }).catch(function () {
                toast('头像已保存到本机，联网后重新保存可同步', true);
              });
            }
          } catch (eSync) { /* 本机保存不受影响 */ }
          renderPage();
          var ev = $('xtpView_edit');
          if (ev && ev.classList.contains('on')) { renderView('edit', ev); }
          toast('头像已更新并保存到本机');
  ...
  function xtpCopyUid() { copyText(userId(), '账号'); }
  ...
  function boot() {
    ...
    var uidBefore = userId();
    loadRemoteMe(function () {
      // 服务端邮箱态 / 账号 / 账号锁定态就绪：若编辑页正打开则刷新其状态
      // （邮箱已绑定 → 只读；账号已锁定 → 只读 + 展示下次可修改时间）
      var ev = $('xtpView_edit');
      if (ev && ev.classList.contains('on')) { renderView('edit', ev); }
      // 服务端账号与本地展示值不同才重绘首页（更新 hero 的「ID：xxx」），相同则不动，避免无谓闪烁。
      // ⚠️ TA 视角（?user=）绝不能重绘本人首页——否则把好友资料页整个覆盖成本人 hero（R142 修复）。
      if (!isOtherMode() && userId() !== uidBefore) { renderPage(); }
    });
  }

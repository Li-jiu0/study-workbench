...
async function api(path, opts) {
  opts = opts || {};
  // 【R9 2026-09-21】请求链路 ID：全链路透传（X-Request-Id）+ 失败落 XTLog，便于 日志.html 检索
  var reqId = '';
  try { if (window.XTLog) { reqId = window.XTLog.newReqId(); window.__xtReqId = reqId; } } catch (e0) { }
  ...
  if (!opts._allowOffline && typeof window.xtNetIsOffline === 'function' && window.xtNetIsOffline()) {
    ...
    try { if (window.XTLog) window.XTLog.warn('api', path + ' → 离线快速失败', 'req=' + reqId); } catch (eL) { }
    throw new Error('网络不可用，请检查网络连接');
  }
  var headers = Object.assign({}, opts.headers || {});
  if (!(opts.body instanceof FormData)) headers['Content-Type'] = 'application/json';
  var t = apiGetToken(); if (t) headers['Authorization'] = 'Bearer ' + t;
  if (reqId) { try { headers['X-Request-Id'] = reqId; } catch (eH) { } }
  ...
  try {
    ...
    if (sig.isTimeout()) {
      try { if (window.XTLog) window.XTLog.warn('api', path + ' → 网络超时', 'req=' + reqId + ' timeout=' + timeoutMs + 'ms'); } catch (eL1) { }
      throw new Error('网络超时，请检查网络后重试');
    }
    if (err && err.name === 'AbortError') throw err;
    try { if (window.XTLog) window.XTLog.warn('api', path + ' → 网络异常', 'req=' + reqId + ' err=' + ((err && err.message) || err)); } catch (eL2) { }
  ...
  if (res.status === 401 && !opts._retried) {
    ...
    try { if (window.XTLog) window.XTLog.warn('api', path + ' → 401 刷新失败强制登出', 'req=' + reqId); } catch (eL3) { }
    apiForceLogout();
  ...
  if (!res.ok) {
    try { if (window.XTLog) window.XTLog.warn('api', path + ' → HTTP ' + res.status, 'req=' + reqId + ' detail=' + ((data && data.detail) || '')); } catch (eL4) { }

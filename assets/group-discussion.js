...
(function () {
  ...
  let gdFollow = true;
  /* 【R11 2026-09-21 用户需求】对话接入真 AI：进行中的 callAI 计数（>0 表示有角色正在思考），
     用于「正在思考发言…」提示与发送按钮防抖。AI 不可用/失败时回退本地模板，功能不中断。 */
  let aiPending = 0;
  /* 会话令牌：每次 select()/back() 自增。在途 AI 回复返回时若令牌已变（用户已换题/退出），
     直接丢弃，避免上一局的发言串进新会话。 */
  let gdSession = 0;
  ...
  // ========== 【R11】AI 发言层 ==========
  // 组装某角色的上下文（题目 + 角色人设 + 近 8 条对话），走 window.callAI（ai-service.js，面测.html 已引入）
  function buildAiMessages(char, userText) {
    const t = currentTopic;
    const others = CHARACTERS.filter(c => c.id !== char.id)
      .map(c => c.name + '（' + c.style + '）').join('；');
    const sys = [
      '你是一场无领导小组讨论（群面）模拟中的真实参与者。题目：' + t.title + '。',
      '题目要求：' + t.desc,
      (t.items && t.items.length) ? ('可选项/物品：' + t.items.join('、') + '。') : '',
      '你扮演「' + char.name + '」，人设与发言风格：' + char.style + '。',
      '其他参与者：' + others + '。用户本人也是参与者之一，请对他的发言做出真实回应（可以赞同、补充、反驳）。',
      '硬性要求：1) 紧扣题目与对话上下文，观点具体、有信息量，不要空话套话；',
      '2) 单次发言控制在 90 字以内，口语化，像真人在群面里说话，不要 markdown、不要编号列表；',
      '3) 只输出你说的话本身，不要带「领导者：」这类前缀，也不要描述动作或表情。'
    ].filter(Boolean).join('\n');
    const msgs = [{ role: 'system', content: sys }];
    const recent = discussionMessages.slice(-8);
    for (let i = 0; i < recent.length; i++) {
      const m = recent[i];
      if (m.role === 'user') msgs.push({ role: 'user', content: m.text });
      else {
        const c = CHARACTERS.find(x => x.id === m.charId);
        msgs.push({ role: 'assistant', content: (c ? c.name : '参与者') + '：' + m.text });
      }
    }
    if (userText) msgs.push({ role: 'user', content: userText });
    /* 【R11j 2026-09-21 用户反馈修复】开场发言时 discussionMessages 为空且 userText 为空，
       messages 只剩 system 一条；服务端把 system 抽成 systemInstruction 后 contents 为空，
       Google 返回 400 INVALID_ARGUMENT「contents is not specified」（页面表现为
       「请求参数不被该模型平台接受」）。此处保证恒有至少一条 user 消息（开场指令）；
       服务端 ai.py 的 _gemini_stream 已同步加空 contents 兜底，双保险。 */
    if (!msgs.some(function (m) { return m.role === 'user'; })) {
      msgs.push({ role: 'user', content: '（讨论开始）请围绕题目做你的开场发言，亮明你的立场。' });
    }
    return msgs;
  }
  ...
  // 「正在思考发言…」提示 + 发送按钮防抖
  function syncAiBusy() {
    const tip = document.getElementById('gdTyping');
    if (tip) tip.classList[aiPending > 0 ? 'add' : 'remove']('on');
    const btn = document.getElementById('gdSendBtn');
    if (btn) { btn.disabled = aiPending > 0; btn.textContent = aiPending > 0 ? '…' : '发送'; }
    if (aiPending > 0) {
  ...
  // 让某角色发一次言：优先真 AI（callAI），失败/不可用回退本地模板（generateCharacterSpeech）
  function aiSpeak(char, userText, fallback) {
    const session = gdSession;
    const stale = function () { return session !== gdSession; };
    const fb = (typeof fallback === 'string' && fallback) ? fallback : generateCharacterSpeech(char, round, currentTopic);
    if (typeof window.callAI !== 'function') { if (!stale()) addCharacterMessage(char, fb); return; }
    aiPending++;
    syncAiBusy();
    window.callAI('auto', buildAiMessages(char, userText), {})
      .then(function (r) {
        if (stale()) return;
        const txt = (r && r.text) ? String(r.text).trim() : '';
        addCharacterMessage(char, txt || fb);
      }, function () {
        if (stale()) return;
        addCharacterMessage(char, fb);
      })
      .then(function () {
        if (stale()) return;
  ...
  /* 【R11】合并页宿主（面测.html）的页面级返回栏：进入讨论后隐藏（把顶部空间还给对话区），
     回到题目列表时恢复。仅当宿主存在时生效，浮层模式无此元素、自动跳过。 */
  function syncMergeBar() {
    const bar = document.getElementById('gdMergeBar');
    if (!bar) return;
    bar.style.display = (currentTopic && isRunning) ? 'none' : '';
    /* 【R11b 用户反馈】讨论区下方还空一大截：pane 高度是进页时（返回栏还显示）量好的，
       进讨论把返回栏藏掉后顶部位置上移，不重算高度就留出一段空白。
       显隐后立即重量高（宿主面测.html 已暴露 window.layoutGdAiPane；浮层模式无此元素不受影响）。 */
    try { if (typeof window.layoutGdAiPane === 'function') window.layoutGdAiPane(); } catch (e) {}
  }
  ...
  function renderTopicList() {
    ...
    return `
      <div class="gd-count gd-count-top">共 ${TOPICS.length} 个经典题目 · 点卡片直接开始</div>
  ...
  function renderDiscussion() {
    ...
    return `
      <div class="gd-disc-view">
      <div class="gd-disc-header">
        <button class="gd-back" onclick="GroupDiscussion.back()">← 返回</button>
      ...
      <div class="gd-messages" id="gdMessages">${msgsHtml}<div class="gd-typing" id="gdTyping">正在思考发言…</div></div>
      <div class="gd-input-area">
        ...
        <button class="gd-send-btn" id="gdSendBtn" onclick="GroupDiscussion.sendUserMessage()">发送</button>
      </div>
  ...
  // ========== 打开无领导小组讨论 ==========
  // 【批次 20260913j】页面内全屏视图：存在宿主容器 #gdAiPane（面测.html 合并页）时，
  ...
  function select(topicId) {
    ...
    bindMessagesScroll();
    syncMergeBar();
    // 第一轮：领导者开场（真 AI，失败回退内置开场白），说完把话头交给用户。
    // session 在排程时捕获：若用户在 400ms 内已换题/退出，到点直接丢弃，不再发言。
    const session = gdSession;
    setTimeout(() => {
      if (session !== gdSession) return;
      aiSpeak(CHARACTERS[0], '', CHARACTERS[0].firstSpeak);
    }, 400);
  }
  ...
  function sendUserMessage() {
    ...
    // 角色回应（随机 1-2 个角色，走真 AI；失败回退本地模板）
    const session = gdSession;
    setTimeout(() => {
      if (session !== gdSession) return;
      const pool = CHARACTERS.slice().sort(() => Math.random() - 0.5);
      const responders = pool.slice(0, 1 + Math.floor(Math.random() * 2));
      responders.forEach((char, i) => {
        setTimeout(() => {
          if (session !== gdSession) return;
          aiSpeak(char, text, '我同意你说的部分观点，不过我想补充一点...' + generateCharacterSpeech(char, round, currentTopic));
        }, i * 1200);
      });
    }, 300);
  ...
  function nextRound() {
    ...
    // 角色继续讨论（全员依次发言，走真 AI；失败回退本地模板）
    const session = gdSession;
    CHARACTERS.forEach((char, i) => {
      setTimeout(() => {
        if (session !== gdSession) return;
        aiSpeak(char, '', generateCharacterSpeech(char, round, currentTopic));
      }, i * 1200);
    });
  ...
  function updateMessages() {
    ...
    // 【R11】重建时补回「正在思考发言…」提示节点（innerHTML 整体替换，提示节点必须随重建重建）
    msgsContainer.innerHTML = msgsHtml + '<div class="gd-typing" id="gdTyping">正在思考发言…</div>';
  ...
  function getStyles() {
    return `
      ...
      /* 【R11】题目列表顶部的轻量说明条（原「随机抽题」按钮行已按用户要求删除，列表直接铺满） */
      .gd-count-top { padding: 14px 20px 2px; flex: 0 0 auto; }
      ...
      /* 2026-09-21 布局重构（用户反馈：辅助信息占比过大、讨论区被挤到角落）：
         讨论态由 .gd-disc-view 接管分工——固定头部紧凑化，.gd-messages 独立滚动拿走
         全部剩余高度；题目物品条与角色条改单行横滑（内容不丢，高度从 3 行压到 1 行）。 */
      .gd-disc-view { display: flex; flex-direction: column; flex: 1; min-height: 0; overflow: hidden; }
      .gd-disc-header { padding: 10px 16px; background: linear-gradient(135deg, rgba(91,141,239,0.1), rgba(91,141,239,0.05)); border-bottom: 1px solid var(--border, #eee); display: flex; align-items: center; gap: 10px; flex: 0 0 auto; }
      ...
      .gd-disc-title { flex: 1; min-width: 0; font-size: 16px; font-weight: 800; color: var(--text, #333); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
      .gd-disc-round { font-size: 12px; padding: 4px 10px; border-radius: 10px; background: var(--primary, #5B8DEF); color: #fff; flex: 0 0 auto; }

      .gd-topic-box { padding: 10px 16px; background: rgba(255,193,7,0.08); border-bottom: 1px solid var(--border, #eee); flex: 0 0 auto; max-height: 34%; overflow-y: auto; }
      .gd-topic-desc { font-size: 13px; line-height: 1.55; color: var(--text, #333); margin-bottom: 8px; }
      .gd-items { display: flex; gap: 6px; flex-wrap: nowrap; overflow-x: auto; -webkit-overflow-scrolling: touch; }
      .gd-item { font-size: 11px; padding: 4px 10px; border-radius: 10px; background: rgba(0,0,0,0.06); color: var(--text-secondary, #666); white-space: nowrap; flex: 0 0 auto; }

      .gd-chars-bar { padding: 6px 16px; display: flex; gap: 8px; flex-wrap: nowrap; overflow-x: auto; -webkit-overflow-scrolling: touch; border-bottom: 1px solid var(--border, #eee); background: var(--bg, #fafafa); flex: 0 0 auto; }
      .gd-char-chip { display: flex; align-items: center; gap: 6px; padding: 4px 10px 4px 4px; border-radius: 20px; border: 1.5px solid; background: var(--bg, #fff); flex: 0 0 auto; }
      ...
      .gd-messages { flex: 1 1 auto; min-height: 0; overflow-y: auto; padding: 12px 16px; display: flex; flex-direction: column; gap: 12px; }
      /* 【R11】AI 发言中的轻提示（挂在 #gdMessages 末尾，随消息区滚动） */
      .gd-typing { display: none; align-self: flex-start; font-size: 12px; color: var(--text-secondary, #888); padding: 6px 12px; border-radius: 10px; background: var(--bg, #f5f5f5); border: 1px dashed var(--border, #ddd); }
      .gd-typing.on { display: block; }
      .gd-input-area, .gd-actions-bar { flex: 0 0 auto; }

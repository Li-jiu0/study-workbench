...
(function () {
  ...
  function push(msg) {
    try {
      ...
      // 2026-09-21 去重补充：系统横幅由安卓壳控制前后台（前台不弹，见 MainActivity.showNotify），
      //   页内卡片在此处做「同会话在屏合并」——同 peerId 卡片仍显示时只刷新文字并重置计时，
      //   不再新挂一条，杜绝「同一条消息两张卡」的重复叠加。
      ...
      if (item.peerId !== null && item.peerId !== undefined) {
        for (var di = 0; di < live.length; di++) {
          var cur = live[di];
          if (cur && !cur.closed && cur.peerId !== null && cur.peerId !== undefined
            && String(cur.peerId) === String(item.peerId)) {
            cur.text = item.text;
            try {
              if (cur.el) {
                var txEl = cur.el.querySelector('.xt-notify-text');
                if (txEl) txEl.textContent = item.text;
              }
            } catch (eU) { /* 刷新失败：保留旧卡不阻断 */ }
            schedule(cur);   // 重置自动消失计时
            return;
          }
        }
      }

...
public class MainActivity extends Activity {
    ...
    private void attachBridgesAndLoad(final WebView w) {
        ...
        w.setWebChromeClient(new WebChromeClient() {
            ...
            /** 【R3b-A / B2】顶部细进度条：加载中显示（水平、不挡操作），走完/到达 100% 隐藏。 */
            @Override
            public void onProgressChanged(WebView view, int newProgress) {
                super.onProgressChanged(view, newProgress);
                try { showProgress(view, newProgress); } catch (Throwable t) { /* 进度条失败不影响加载 */ }
            }
        ...
        // 【R3b-A / B2】顶部进度条容器（懒建）：把细进度条叠在 WebView 之上，覆盖整页加载过程。
        ensureProgressBar();

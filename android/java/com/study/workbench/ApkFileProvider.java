...
/**
 * 星途 · 应用内更新包安装用极简 FileProvider（R101）
 * ---------------------------------------------------------------
 ...
 *   ContentProvider：把「系统公共 Download 目录下的 .apk」以 content://
 *   形式授权给系统包安装器读取。
 *
 * URI 约定：content://com.study.workbench.apkfile/<文件名>
 *   Android 7+（N）直接对 file:// 跨进程读会抛 FileUriExposedException，
 *   安装 APK 必须走 content:// + FLAG_GRANT_READ_URI_PERMISSION。
 *
 * 安全约束（防目录穿越，见 resolveCheckedFile）：
 *   - 只接受纯文件名（拒绝包含 / \ .. 的路径段）
 *   - 规范化（canonical）后必须仍位于公共 Download 目录内（防符号链接绕过）
 *   - 只开放 .apk 扩展名
 */
public class ApkFileProvider extends ContentProvider {

    /** 与 AndroidManifest.xml 中 <provider android:authorities> 保持一致 */
    public static final String AUTHORITY = "com.study.workbench.apkfile";

    /** 把公共 Download 目录下的文件包装成本 provider 的 content:// URI（文件名做 URL 编码） */
    ...
    /** 包安装器可能查询 DISPLAY_NAME / SIZE（OpenableColumns）；仅支持这两列，其余列返回 null */
    @Override
    ...
    private File resolveCheckedFile(Uri uri) throws FileNotFoundException {
        ...
        // 二次校验：规范化后必须仍在 Download 目录内（防符号链接等绕过）
        try {
            String canon = f.getCanonicalPath();
            String dirCanon = downloads.getCanonicalPath() + File.separator;

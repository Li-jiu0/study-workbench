...
async def _gemini_stream(cfg: dict, model_name: str, fallback: bool, messages: list,
    ...
    """Gemini 流式中转：OpenAI messages → Gemini contents，SSE 逐 chunk 透传。

    - 格式转换：system → systemInstruction；user/assistant → contents（role 映射
      user/model）；key 放 URL 参数 ?key=。
    - 【R6 2026-09-20】由 generateContent 一次性聚合改为 streamGenerateContent(alt=sse)
      流式透传——旧实现等上游整段生成完才下发一帧，用户看到思考内容时模型早已
      思考完。现逐 chunk 解析 candidates[0].content.parts[]：
      part.thought=true → _sse_frame("r", text)（思维链，不入 acc/历史，与 R107 口径一致）；
      其余 part.text → _sse_frame("c", text) 并计入 acc。
    - 深度思考：body.reasoning=true 时 generationConfig.thinkingConfig.includeThoughts=True
      （Gemini 思考模型专属字段；非思考模型忽略之，不另做白名单——Gemini 仅此一家上游）。
    - use_frames=False（未开深度思考）时保持旧口径：纯文本增量逐 chunk 下发。
    ...
    # 【R11j 2026-09-21 修复】上游 400 "contents is not specified" 兜底：若调用方只传了
    # system 消息（如无领导小组讨论开场发言：discussionMessages 为空且 userText 为空，
    # messages 仅剩 system 一条），上面的映射会得到空 contents，Google 必拒。
    # 这里把 system 文本降级为首条 user 轮，保证请求恒合法；有正常 user/assistant 时零影响。
    ...
    if use_frames:
        # 深度思考开启：要求上游在流里回传 thought parts（仅思考模型实际返回）
        gen_cfg["thinkingConfig"] = {"includeThoughts": True}
    if gen_cfg:
        payload["generationConfig"] = gen_cfg
    # alt=sse：streamGenerateContent 默认返回 JSON 数组流，alt=sse 才是标准 SSE 行分帧
    url = f"{cfg['base_url'].rstrip('/')}/{model_name}:streamGenerateContent"
    try:
            ...
            async with client.stream(
                ...
                async for line in resp.aiter_lines():
                    ...
                    try:
                        ...
                        continue  # 心跳/注释行等跳过
                    try:
                        # token 账本：usageMetadata 通常只在末尾 chunk 出现（取最大值幂等）。
                        # 口径与旧实现一致：prompt + candidates（thoughtsTokenCount 不计，
                        # 避免与既有日账口径跳变）。
                        ...
                        for part in parts:
                            ...
                            if use_frames:
                                if part.get("thought"):
                                    # 思维链帧：不进 acc（AiLog 只存正文）
                                    yield _sse_frame("r", piece)
                                else:
                                    acc.append(piece)
                        ...
                        continue  # 单 chunk 异常不炸整流
        # 正常收尾（错误路径上方 return / except 不收尾，维持裸文本）
        if use_frames:
            if used_tokens > 0:
                yield (b"data: "
...
async def chat(body: _ChatIn, request: Request, user: User = Depends(get_current_user_optional),
    ...
    display_model_name = (resolved_name
                          ...
                          or model_name)
    # R132：Gemini 走专属格式转换；【R6 2026-09-20】已改 streamGenerateContent(SSE) 流式透传（见 _gemini_stream）
...
async def video_generate(body: dict, request: Request,
    ...
    入参：{"modelId": "ark-seedance-2-0", "prompt": "...",
...
async def model3d_task(task_id: str, request: Request,
    ...
    if view["status"] == "succeeded" and not model_url:
        return _err("provider_error", "3D 生成完成但未返回文件地址，请稍后再试", code=502,
                    detail=_upstream_msg(resp))
    # 【R6 2026-09-20】生成即解包：任务一旦成功且结果是 zip，立刻在服务端解包
    # （趁 24h 链接新鲜），结果写入 meta.json 缓存；前端随后调 /model3d/preview
    # 必命中缓存秒回，历史消息隔天再看也不再受链接过期影响。
    # 线程池执行避免阻塞事件循环；解包失败静默——前端仍会走 preview 端点重试/降级。
    if (view["status"] == "succeeded" and model_url
            and model_url.lower().split("?")[0].endswith(".zip")):
        import asyncio
        try:
            await asyncio.to_thread(unpack_model3d_zip, model_url)
        except Exception:
            pass
...
# 【R6 2026-09-20】白名单补火山引擎全家桶域：实测 ark 3D 结果包可能落在
# TOS 主域或其 CDN 域（volcengine/volccdn/volcimg/volcengineapi），旧表只放
# volces.com 时会误判「结果包地址不在允许的存储域内」→ 前端只剩 zip 下载。
# SSRF 防护不变：仍要求 https + DNS 解析非内网（_model3d_assert_safe_url）。
_MODEL3D_HOST_ALLOW = ("volces.com", "volcengine.com", "volccdn.com",
                       "volcimg.com", "volcengineapi.com")

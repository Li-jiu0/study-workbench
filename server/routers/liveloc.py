...
class StartIn(BaseModel):
    ...
    silent：R3·L2 新增（可选，默认 false）。为 true 时不落「开始共享实时位置」系统卡片，
    专供 live-location.html「点进即加入」的隐式 start —— 接收方点一次「查看」即自动加入共享，
    若每次都落卡片会在聊天流刷屏（验收标准 3：反复进/出页面不重复落卡片）。
    显式「发起共享」（加号菜单）仍走 silent=false，卡片语义不变。
    """
...
async def live_start(body: StartIn, user: User = Depends(get_current_user),
    ...
    - 副作用：往消息流落 1 张 kind=location_live 系统卡片（**不含坐标**），私聊/群聊各走对应路径；
      silent=true 时**不落卡片**（供隐式「点进即加入」用，见 StartIn.silent 注释）。
    """
    ...
    # silent=true（隐式「点进即加入」）→ 不落卡片，避免反复进出刷屏；显式发起仍落卡片。
    if not body.silent:
        if peer:

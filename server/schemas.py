...
# ---------- 账号（account）（R141 2026-09-21；R142 口径纠偏 2026-09-21） ----------
# 语义（用户明确）：「账号」= 用户注册 / 登录所用的那个账号，30 天可自助改一次。
# 规则与注册账号完全一致（server/routers/auth.py 的 _USERNAME_RE）：
#   3~20 位「中文 / 字母 / 数字 / 下划线」。
# 不做大小写折叠：注册时用户怎么写就怎么展示（保证「ID 位置显示的就是注册账号」原样）。
# 唯一性 / 登录匹配统一按「忽略大小写」比较（见 routers/users.update_account、
# routers/auth.login），避免 Alice 与 alice 被当成两个账号。
_ACCOUNT_RE = re.compile(r"^[一-龥A-Za-z0-9_]{3,20}$")


def check_account_format(account: str) -> str:
    """账号格式校验 + 去空白（保留原大小写）；非法抛 ValueError（→ pydantic 422）。

    与前端 assets/xt-profile.js 的 xtValidateAccount 正则保持一致：
      3~20 位、中文 / 字母 / 数字 / 下划线。
    """
    a = (account or "").strip()
    if not _ACCOUNT_RE.match(a):
        raise ValueError("账号需为 3-20 位，可含中文、字母、数字与下划线")
    return a


class AccountIn(BaseModel):
    """PUT /api/users/me/account：修改账号（30 天一次，服务端权威校验）。"""
    account: str = Field(min_length=1, max_length=20)

    @field_validator("account")
    @classmethod
    def _check_account(cls, v: str) -> str:
        return check_account_format(v)
...
class ReplyIn(BaseModel):
    """R3b-C U9：引用快照入参（可选）。私聊 / 群聊共用。

    - toId：被引用消息 id（服务端据此校验「存在且属于同一会话」）；
    - senderName / kind / text：冗余最小快照，仅作展示兜底；服务端优先以真实被引用消息重取。
      text 限长 100。
    """
    toId: int | None = None
    senderName: str = Field(default="", max_length=64)
...
class GroupMsgIn(BaseModel):
    ...
    # R3b-C U9：可选引用快照（与私聊同口径）；旧前端不传 → None，行为零变化。

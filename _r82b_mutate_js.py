# -*- coding: utf-8 -*-
"""R82 第二段：assets/xt-moments.js（ico 助手 + 动态图标）与 assets/xt-moments.css（.xtm-ico）。
HTML 段已由 _r82a_mutate.py 落盘；备份 *.bak-pre-r82-20260917 亦已生成。勿重跑 _r82a。"""
import os
...
# ---------- 1. assets/xt-moments.js ----------
jp = P(r'assets\xt-moments.js')
j = rb(jp)
assert crlf_ok(j), 'xt-moments.js bareLF'
assert j.count('function ico('.encode('utf-8')) == 0, '已含 ico()，勿重复执行'

# 1a. 新增 ico() 助手（IIFE 内私有，不产生全局名）
...
# 1c. 卡片操作行：❤/🤍 → heart，💬 → message-circle
j = rep(j, "(m.likedByMe ? '❤ 取消' : '🤍 赞')".encode('utf-8'),
        "(m.likedByMe ? ico('heart', 13, '❤') + ' 取消' : ico('heart', 13, '🤍') + ' 赞')".encode('utf-8'), 1)
j = rep(j, 'onclick="XTM.panel(\' + m.id + \')">💬 评论</span>\' +'.encode('utf-8'),
        'onclick="XTM.panel(\' + m.id + \')">\' + ico(\'message-circle\', 13, \'💬\') + \' 评论</span>\' +'.encode('utf-8'), 1)

# 1d. 关闭符 ✕ → close（弹层×5 / 查看器 / 发布格×2）
j = rep(j, '>✕</span></div>\' +'.encode('utf-8'), '>\' + ico(\'close\', 18, \'✕\') + \'</span></div>\' +'.encode('utf-8'), 5)
...
# 1f. R78 段注释里的 emoji 指涉改写为图标名
j = rep(j, '     #xtmHero 存在时启用：🖼 选本地图片（≤2MB）→ FileReader dataURL → localStorage'.encode('utf-8'),
        '     #xtmHero 存在时启用：两按钮图标为 lucide image / rotate-ccw；选本地图片（≤2MB）→ FileReader dataURL → localStorage'.encode('utf-8'), 1)
...
assert j.count('ico('.encode('utf-8')) >= 10, 'ico() 调用数不足: %d' % j.count('ico('.encode('utf-8'))
assert crlf_ok(j)
wb(jp, j)
log('UPDATE assets/xt-moments.js bytes=%d' % len(j))

# ---------- 2. assets/xt-moments.css ----------
cp = P(r'assets\xt-moments.css')
c = rb(cp)
assert crlf_ok(c), 'xt-moments.css bareLF'
assert c.count('.xtm-ico'.encode('utf-8')) == 0, '已含 .xtm-ico，勿重复执行'
anchor = '/* ---------- 响应式（窄屏断点） ---------- */'.encode('utf-8')
new_css = blk([
    '/* ---------- R82：动态插入的 lucide 图标容器（xt-moments.js ico() 使用） ----------',
    '   与静态 data-icon 共用同一套 LUCIDE_ICONS 字典；stroke 为 currentColor，随主题/父级文字色变化。',
    '   固定值对齐：vertical-align:-3px 让 12–13px 图标与 13px 正文基线齐平。 */',

...
(async function () {
  ...
  assert('E2 所有分片请求均带缓存版本 ?v=20260913g', shardReqs.length > 0 && shardReqs.every(function (u) { return u.indexOf('?v=20260913g') >= 0; }),
    '样例 ' + shardReqs.slice(0, 2).join(' | '));
  assert('E3 词库索引请求带 ?v=20260913g', flog.some(function (u) { return u.indexOf('vocab-cet4-ext-index.json?v=20260913g') >= 0; }), '');
  ...
  assert('E5 题库 legacy 请求带 ?v=20260913g', legacyReqs.length === 1 && legacyReqs[0].indexOf('?v=20260913g') >= 0, legacyReqs.join(', '));

...
blocked = []
for pat, name in [(b'<REDACTED-GEMINI-旧KEY前缀>', 'Gemini AQ.'), (b'<REDACTED-OPENROUTER-旧KEY前缀>', 'OpenRouter sk-or-v1')]:
    if pat.encode() if isinstance(pat, str) else pat in patch.encode('utf-8', 'ignore'):
        blocked.append(name)
# 上面的写法有歧义，重新严格扫：
blocked = []
patch_b = patch.encode('utf-8', 'ignore')
for pat, name in [('<REDACTED-GEMINI-旧KEY前缀>', 'Gemini AQ.'), ('<REDACTED-OPENROUTER-旧KEY前缀>', 'OpenRouter sk-or-v1')]:
...
for pat, name in [('<REDACTED-ARK-旧KEY前缀>', 'ARK'), ('<REDACTED-ZHIPU-旧KEY前缀>5685', 'ZHIPU'), ('bce-v3/<REDACTED-ALTAK>', 'QIANFAN')]:

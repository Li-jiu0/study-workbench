# -*- coding: utf-8 -*-
"""20260916c 部署：AI 问答页全套（新页首次上线 + 底座倍率/MAX/自动路由）
- AI.html（新页面，首次上线）
...
for must in ['web/assets/app.js', 'web/更多.html', 'web/私聊.html']:
    assert must in names, '断言失败 ' + must
LOG.append('关键文件断言 OK')

if os.environ.get('SW_DRY_RUN') == '1':
    io.open(r'C:\Users\ATM\_dep_20260916c_out.txt', 'w', encoding='utf-8').write('\n'.join(LOG))
    sys.exit(0)

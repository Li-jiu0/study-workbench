...
# 1) 上传 config.py
for rel, dst in [('server/config.py', 'server/config.py'),
                 ('server/security.py', 'server/security.py'),
                 ('server/routers/ai.py', 'server/routers/ai.py')]:
    r = subprocess.run([PSCP, '-pw', pwd, '-batch', '-hostkey', HOSTKEY,
                        os.path.join(ROOT, *rel.split('/')), 'root@' + host + ':/opt/study-workbench/' + dst],
                       capture_output=True, text=True, timeout=120, errors='replace')
    LOG.append('%s 上传 exit=%d %s' % (rel, r.returncode, (r.stderr or '')[-150:]))

# 2) .env 追加 Key（幂等：存在即跳过）
SF_KEY = '***REMOVED-BY-R2C***'
ZP_KEY = 'f5aa78850e7a4575a1c4d119b03c3019.Uvg73xeMhVdN7crs'
append_cmd = (

# -*- coding: utf-8 -*-
"""20260913n 部署：R31-R36 前端批次上线（纯前端，不动 server/）。

与 deploy_update_20260913m.py 的差异（安全机制逐条照搬，仅去掉 server 侧步骤）：
  1. STAMP / 包名 / DB 备份名统一为 20260913n；探针版本串
     CET_VER_K/HOME_VER_K -> CET_VER_N/HOME_VER_N（阈值不变，均 >=10）。
  2. 保留：blog_wechat.html 排除、ROOT 铁律校验（含 worktree 路径检测）、
     三条资源硬断言（icon-map.js/subpage-router.js/mock-papers.js）、
     探针剥注释逻辑、SW_DRY_RUN/SW_SKIP_UPLOAD 机制、全量 MD5、凭据惰性读取、
     USERS=6 基线。
  3. **移除 server 侧步骤**（本批无任何 server/ 改动）：
     - 不传 server 文件、不校验 main.py include、不改远端 .env、
       不重启后端服务（STEP-SERVER-RESTART 整段删除）；
     - 探针去掉 FEEDBACK_POST（属后端接口，与前端批次无关）；
     - NRESTARTS 由「硬断言==0」降级为 INFO（前端批次不重启服务，
       该计数可能来自历史重启，不作为放行门槛；放行门槛为 SERVICE active + HEALTH 200）。
  4. **新增本批内容探针（证明 R31-R36 改动已上线）**：
     - R33：私聊.html 含 `.im-tab.active{background:var(--primary);color:#fff`
     - R34：设置.html 含 `ST_ACCT_RETRIES`（账号检测重试机制）
     - R36：设置.html 含 `每日复习量`（每日复习量设置项）
  5. 保留旧批次回归探针：B1-B4 删减计数=0、模考三页禁 history.back()、首页 data-icon>=1、
     E1 图标 span>=1。

前端(web/)：
  31 个 HTML 页面（排除 blog_wechat.html）+ assets/*.js/*.css
  + assets/data/*.json（含词库扩展索引+8分片）+ assets/emoji/manifest.js + 根 data/。
  server/ 不打包（无改动）。

安全机制：
  - 逐文件 MD5 全量比对（非抽查）；md5sum 输出解析为 {name: hash}（勿写 dict(...)）；
  - 凭据惰性读取（仅真正部署时）；--dry-run 永不触碰；
  - ASCII 包名中转：只上传 deploy_20260913n.tar.gz，规避 pscp GBK->UTF-8 中文名错乱。
  - DB 备份仍执行（无害，纯预防）。

用法：
  $env:SW_DRY_RUN=1;  python tools/deploy_update_20260913n.py   # 本地清单+打包，不联网
  $env:SW_SKIP_UPLOAD=1; python tools/deploy_update_20260913n.py  # 需凭据：解压+全量校验+探针
  python tools/deploy_update_20260913n.py                       # 正式部署（含上传+重启服务）
"""
import os
import sys
import io
...
MANIFEST_PATH = os.path.join(ROOT, "tools", "qa", "_deploy_0913n_manifest.txt")
...
print("CET_VER_N:" + str(cnt("\u56db\u7ea7\u5907\u8003.html", r"20260913n")))
print("HOME_DATAICON:" + str(cnt("\u5b66\u4e60\u5de5\u4f5c\u53f0.html", r"data-icon=")))
print("HOME_VER_N:" + str(cnt("\u5b66\u4e60\u5de5\u4f5c\u53f0.html", r"20260913n")))
...
def judge_probes(out):
    ...
    if r33 is None or r33 < 1:
        fatal.append("R33 私聊.html 未含 .im-tab.active{background:var(--primary);color:#fff（计数=%s）" % r33)
    else:
        info.append("R33 私聊 tab 样式已上线（计数=%s）" % r33)
    r34 = num("R34_ACCT_RETRY")
    if r34 is None or r34 < 1:
        fatal.append("R34 设置.html 未含 ST_ACCT_RETRIES（账号检测重试机制缺失，计数=%s）" % r34)
    else:
        info.append("R34 账号检测重试机制已上线（计数=%s）" % r34)
    r36ui = num("R36_DAILY_REVIEW_UI")
    if r36ui is None or r36ui < 1:
        fatal.append("R36 设置.html 未含「每日复习量」设置项（计数=%s）" % r36ui)
    else:
        info.append("R36 每日复习量设置项已上线（计数=%s）" % r36ui)
    r36js = num("R36_DAILY_REVIEW_JS")
    if r36js is None or r36js < 1:
        fatal.append("R36 设置.html 未含 dailyReview（计数=%s）" % r36js)
    else:
        info.append("R36 dailyReview 引用已上线（计数=%s）" % r36js)

    cv = num("CET_VER_N")
    if cv is None or cv < 10:
        fatal.append("四级备考.html 版本串 20260913n 计数=%s（期望 >=10：漏 bump 会命中旧缓存）" % cv)
    hv = num("HOME_VER_N")
    if hv is None or hv < 10:
        fatal.append("首页 学习工作台.html 版本串 20260913n 计数=%s（期望 >=10）" % hv)

...
校验项（1.41 口径）：
  A 版本：aapt2 dump badging → versionCode=42 / versionName=1.41
  B 白名单：build_apk.REQUIRED_ASSETS 全部在包内
  C 页面：包内 HTML 数量、关键新页（日志.html/更新.html/关于.html）
  D 内容：xt-update.js CURRENT_VERSION='1.41'
  E 安全：包内所有 .js 零明文 Gemini key（AQ. / AIza）
  F 新模型：ai-config.js 含 ark-seedream-5-0 / ark-hyper3d-gen2 / ark-seedance-1-0-pro
  G 戳：面测.html=20260925b、日志.html=20260925d、关于.html 的 xt-update.js=20260926a、
        私聊.html 的 chat-local.js=20260926b、个人资料.html 的 xt-profile.js=20260926c；协议.html 含 V1.41
  H resources.arsc 必须 STORED 且 4 字节对齐
  I R144：live-location.html 含多点连线/距离标注/导航入口实现
"""
...
ck('A1 versionCode=42', bool(m) and m.group(1) == '42', '(got %s)' % (m.group(1) if m else '?'))
m = re.search(r"versionName='([^']+)'", badging)
ck('A2 versionName=1.41', bool(m) and m.group(1) == '1.41', '(got %s)' % (m.group(1) if m else '?'))
...
ck('D1 CURRENT_VERSION=1.41', bool(m) and m.group(1) == '1.41', '(got %s)' % (m.group(1) if m else '?'))
...
ck('G3 关于.html xt-update.js=20260926a', stamp_of('关于.html', 'assets/xt-update.js') == '20260926a',
   str(stamp_of('关于.html', 'assets/xt-update.js')))
ck('G4 协议.html 含 V1.41', 'V1.41' in zf.read('assets/协议.html').decode('utf-8', 'replace'))
ck('G5 私聊.html chat-local.js=20260926b', stamp_of('私聊.html', 'assets/chat-local.js') == '20260926b',
   str(stamp_of('私聊.html', 'assets/chat-local.js')))
ck('G6 个人资料.html xt-profile.js=20260926c', stamp_of('个人资料.html', 'assets/xt-profile.js') == '20260926c',
   str(stamp_of('个人资料.html', 'assets/xt-profile.js')))

# ---------- I R144 位置共享增强（包内 live-location.html 必须含新实现） ----------
ll = zf.read('assets/live-location.html').decode('utf-8', 'replace')
for key in ['function renderLinks', 'stableOrderPts', 'llDistIcon', 'navUrlFor',
            'uri.amap.com/navigation', 'll-nav', 'll-sheet']:
    ck('I R144 live-location.html 含 %s' % key, key in ll)

/* AI page sidebar click navigation check (jsdom, local current code).
   ...
   ASCII-only source; Chinese via \uXXXX escapes. Report: tools/qa/_r7_ai_click_file_report.txt */
'use strict';
const fs = require('fs');
...
const PAGE = 'AI.html';
const OUT = path.join(ROOT, 'tools', 'qa', '_r7_ai_click_file_report.txt');
const report = [];
...
(async function main() {
  ...
  const opened = /\bopen\b/.test(afterOpen);
  ...
  const nav3 = navAttempts.filter(function (m) { return /ai-settings\.html/.test(m); });
  ...
  click('aiCustomEntry');
  await wait(300);
  const nav4 = navAttempts.filter(function (m) { return /ai-settings\.html/.test(m); });
  w('  navigation attempts: ' + (navAttempts.length ? navAttempts.join(' | ') : '(none)'));

# -*- coding: utf-8 -*-
"""2026-09-13h 部署：P0 批次上线（B1-B4 功能删减 + C 线修复 + E1 图标全站升级 + 引用驱动补 bump）

前端(web/)：
  32 个 HTML 页面（本批补 bump 后 26 页为 20260913h；学途.html 保持 g、5 个不加载
   本批变更 assets 的页面保持 20260913a——见 tools/qa/_deploy_0913h_manifest.txt 明细）
  + assets/ 下全部 *.js / *.css
  + assets/data/*.json（glob 自动纳入，显式排除字段补产中间产物）
  + assets/emoji/manifest.js
  + 根目录 data/（递归收集；本批含 data/mock-papers.js）

本批 server/ 零改动 → 不部署 server 文件、**不重启 study-workbench 服务**
（Nginx 直接读静态目录 /opt/study-workbench/web，tar 解压即生效）。

与 deploy_update_20260913d.py 的差异（其余安全机制逐条照搬）：
  1. **ROOT 修正为项目主副本 D:\\下载的文件\\学习工作台**（0913d 误指向
     C:\\Users\\ATM\\WorkBuddy\\Worktrees\\...\\main-8d0a1649 旧 worktree——实测该
     worktree 仍含已删除功能且无 E1 图标，直接部署会整体回退本批成果）；
     支持环境变量 SW_ROOT 覆盖。
  2. STAMP / 包名 / 远端包名 / DB 备份名统一为 20260913h。
  3. **补回三条硬断言**：assets/icon-map.js、assets/subpage-router.js（0913d 缺失）
     与 data/mock-papers.js，缺失即 SystemExit。
  4. 探针升级（全部 SOFT-WARN 汇总后一次性 FATAL，避免逐条误杀）：
     - 删减探针**先剥离 HTML 注释与 <script> 内 JS 注释**再计数（实测 mock_exam.html /
       mock_exam_result.html 的 javascript:history.back() 各 1 处为文档注释，raw 计数会误报）；
     - B1/B2：四级备考 openMiniQuiz('cet-write')、央国企笔试 openMiniQuiz('exam-quant'|
       'exam-deduce'|'exam-data') 的可执行调用数必须为 0；
     - B4：工具.html 两个删除入口的跳转与标题必须为 0；
     - B3：高情商表达 eqQuizCard / 情景选择实战 必须为 0；
     - 模考三页：javascript:history.back() 可执行数 0 且 onclick="goBack >=1；
     - 版本探针：四级备考.html '20260913h' >=10；首页 data-icon>=1 且 '20260913h'>=10；
     - E1：blog_wechat bn-icon 图标 span >=1、工具 mpc-icon span >=1。
  5. 支持 SW_DRY_RUN=1（本地清单 + 打包 + 打印，绝不联网/不读凭据）
     与 SW_SKIP_UPLOAD=1（跳过 pscp，仅远端解压+校验，用于已上传复跑）。

安全机制（与 0913d 一致）：
  - 逐文件 MD5 **全量**比对（非抽查）；md5sum 输出解析为 {name: hash}（勿写 dict(...)）；
  - 凭据**惰性读取**（仅真正部署时），--dry-run 永不触碰；
  - **ASCII 包名中转**：只上传 deploy_20260913h.tar.gz，服务器端 tar xzf 落盘，
    规避 pscp 的 GBK→UTF-8 中文名错乱；
  - 排除清单：*.bak-* / *.tar.gz / tools/ 等（本脚本只收集白名单目录）。

用法：
  $env:SW_DRY_RUN=1;  python tools/deploy_update_20260913h.py   # 本地清单+打包，不联网
  $env:SW_SKIP_UPLOAD=1; python tools/deploy_update_20260913h.py  # 需凭据：解压+全量校验+探针
  python tools/deploy_update_20260913h.py                       # 正式部署（含上传）
"""
import os
import sys
import io
...
MANIFEST_PATH = os.path.join(ROOT, "tools", "qa", "_deploy_0913h_manifest.txt")
...
print("CET_VER_H:" + str(cnt("\u56db\u7ea7\u5907\u8003.html", r"20260913h")))
print("HOME_DATAICON:" + str(cnt("\u5b66\u4e60\u5de5\u4f5c\u53f0.html", r"data-icon=")))
print("HOME_VER_H:" + str(cnt("\u5b66\u4e60\u5de5\u4f5c\u53f0.html", r"20260913h")))
...
def judge_probes(out):
    ...
    if cv is None or cv < 10:
        fatal.append("四级备考.html 版本串 20260913h 计数=%s（期望 >=10：漏 bump 会命中旧缓存）"
                     % cv)
    hv = num("HOME_VER_H")
    if hv is None or hv < 10:
        fatal.append("首页 学习工作台.html 版本串 20260913h 计数=%s（期望 >=10）" % hv)
...
def main():
    ...
    print("[0] ROOT=%s" % ROOT)
    print("[0] 页面 %d + assets(js/css) %d + assets/data.json %d + emoji %d + 根data %d "
          "= %d 个文件"

...
与 deploy_update_20260913i.py 的差异（其余安全机制逐条照搬）：
  1. STAMP / 包名 / 远端包名 / DB 备份名统一为 20260913l；探针版本串
...
安全机制（与 0913i 一致）：
  ...
  - **ASCII 包名中转**：只上传 deploy_20260913l.tar.gz，服务器端 tar xzf 落盘，
    规避 pscp 的 GBK→UTF-8 中文名错乱；
  - 排除清单：*.bak-* / *.tar.gz / tools/ 等（本脚本只收集白名单目录）。

用法：
  $env:SW_DRY_RUN=1;  python tools/deploy_update_20260913l.py   # 本地清单+打包，不联网
  $env:SW_SKIP_UPLOAD=1; python tools/deploy_update_20260913l.py  # 需凭据：解压+全量校验+探针
  python tools/deploy_update_20260913l.py                       # 正式部署（含上传+重启服务）
"""
import os
import sys
import io
...
MANIFEST_PATH = os.path.join(ROOT, "tools", "qa", "_deploy_0913l_manifest.txt")
...
print("CET_VER_K:" + str(cnt("\u56db\u7ea7\u5907\u8003.html", r"20260913l")))
print("HOME_DATAICON:" + str(cnt("\u5b66\u4e60\u5de5\u4f5c\u53f0.html", r"data-icon=")))
print("HOME_VER_K:" + str(cnt("\u5b66\u4e60\u5de5\u4f5c\u53f0.html", r"20260913l")))
...
def judge_probes(out):
    ...
    if cv is None or cv < 10:
        fatal.append("四级备考.html 版本串 20260913l 计数=%s（期望 >=10：漏 bump 会命中旧缓存）"
                     % cv)
    hv = num("HOME_VER_K")
    if hv is None or hv < 10:
        fatal.append("首页 学习工作台.html 版本串 20260913l 计数=%s（期望 >=10）" % hv)

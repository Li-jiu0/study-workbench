# -*- coding: utf-8 -*-
"""20260913o 部署：R31-R36 前端批次上线（纯前端，不动 server/）。

与 deploy_update_20260913m.py 的差异（安全机制逐条照搬，仅去掉 server 侧步骤）：
  1. STAMP / 包名 / DB 备份名统一为 20260913o；探针版本串
...
安全机制：
  ...
  - ASCII 包名中转：只上传 deploy_20260913o.tar.gz，规避 pscp GBK->UTF-8 中文名错乱。
  - DB 备份仍执行（无害，纯预防）。

用法：
  $env:SW_DRY_RUN=1;  python tools/deploy_update_20260913o.py   # 本地清单+打包，不联网
  $env:SW_SKIP_UPLOAD=1; python tools/deploy_update_20260913o.py  # 需凭据：解压+全量校验+探针
  python tools/deploy_update_20260913o.py                       # 正式部署（含上传+重启服务）
"""
import os
import sys
import io
...
MANIFEST_PATH = os.path.join(ROOT, "tools", "qa", "_deploy_0913o_manifest.txt")

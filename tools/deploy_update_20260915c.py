# -*- coding: utf-8 -*-
"""20260915c 部署：批次八首波 —— 申论模块上线 + bug 修复批 + 页面合规批（纯前端，不重启服务）。
...
本批内容（全部前端，server/ 无改动，因此**不需要**重启 FastAPI）：
  ...
  改动：
    ...
    - 23 个页面侧边栏新增「申论刷题」入口
  版本戳：统一 bump 到 20260915c（98 处 / 33 文件）
...
用法：
  $env:SW_DRY_RUN=1;  python tools/deploy_update_20260915c.py
  python tools/deploy_update_20260915c.py
"""
